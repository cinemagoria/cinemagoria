...
export default defineEventHandler(async (event) => {
    ...
    try {
        if (type === 'person') {
            ...
            // Fallback for no type specified (legacy behavior support if needed, but safer to be explicit)
            // We will try to fetch for all if type is missing, mimicking previous behavior but separated
            if (tmdbId) {
