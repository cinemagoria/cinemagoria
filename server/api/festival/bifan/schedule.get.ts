...
export default defineEventHandler(async (event) => {
    ...
    try {
        const sql = `
            ...
            WHERE f.festival_name = 'BIFAN' AND f.festival_year = 2026
        ...
        console.error('BIFAN 2026 Schedule Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch BIFAN 2026 schedule: ${error.message}`,
        })
    }
})
