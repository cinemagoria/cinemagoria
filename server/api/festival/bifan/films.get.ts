...
export default defineEventHandler(async (event) => {
    ...
    try {
        let sql = `SELECT * FROM festival_films WHERE festival_name = 'BIFAN' AND festival_year = 2026`
        ...
        console.error('BIFAN 2026 Films Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch BIFAN 2026 films: ${error.message || error}`,
        })
    }
})
