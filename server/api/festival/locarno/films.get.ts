...
export default defineEventHandler(async (event) => {
    ...
    try {
        let sql = `SELECT * FROM festival_films WHERE festival_name = 'Locarno Film Festival' AND festival_year = 2026`
        ...
        console.error('Locarno Film Festival 2026 Films Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch Locarno Film Festival 2026 films: ${error.message || error}`,
        })
    }
})
