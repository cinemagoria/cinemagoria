...
export default defineEventHandler(async (event) => {
    ...
    try {
        const sql = `
            ...
            WHERE f.festival_name = 'Fantasia International Film Festival' AND f.festival_year = 2026
        ...
        console.error('FANTASIA Schedule Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch FANTASIA schedule: ${error.message}`,
        })
    }
})
