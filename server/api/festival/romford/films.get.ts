...
export default defineEventHandler(async (event) => {
    ...
    try {
        let sql = `SELECT * FROM festival_films WHERE festival_name = 'Romford Horror Festival' AND festival_year = 2026`
        ...
        console.error('Romford Films Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch Romford films: ${error.message || error}`,
        })
    }
})
