...
export default defineEventHandler(async (event) => {
    ...
    try {
        const sql = `
            ...
            WHERE f.festival_name = 'Romford Horror Festival' AND f.festival_year = 2026
        ...
        console.error('Romford Schedule Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch Romford schedule: ${error.message}`,
        })
    }
})
