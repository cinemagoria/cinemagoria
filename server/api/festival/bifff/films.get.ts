...
export default defineEventHandler(async (event) => {
    ...
    try {
        let sql = `SELECT * FROM festival_films WHERE festival_name = 'BIFFF' AND festival_year = 2026`
        ...
        console.error('BIFFF Films Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch BIFFF films: ${error.message || error}`,
        })
    }
})
