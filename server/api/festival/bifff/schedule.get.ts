...
export default defineEventHandler(async (event) => {
    ...
    try {
        const sql = `
            ...
            WHERE f.festival_name = 'BIFFF' AND f.festival_year = 2026
        ...
        console.error('BIFFF Schedule Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch BIFFF schedule: ${error.message}`,
        })
    }
})
