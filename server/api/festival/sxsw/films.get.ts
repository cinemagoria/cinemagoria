...
export default defineEventHandler(async (event) => {
    ...
    try {
        let sql = `SELECT * FROM festival_films WHERE festival_name = 'SXSW Film & TV Festival' AND festival_year = 2026`
        ...
        console.error('SXSW Films Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch SXSW films: ${error.message || error}`,
        })
    }
})
