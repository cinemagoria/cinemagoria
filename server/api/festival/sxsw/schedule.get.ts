...
export default defineEventHandler(async (event) => {
    ...
    try {
        const sql = `
            ...
            WHERE f.festival_name = 'SXSW Film & TV Festival' AND f.festival_year = 2026
        ...
        console.error('SXSW Schedule Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch SXSW schedule: ${error.message}`,
        })
    }
})
