...
export default defineEventHandler(async (event) => {
    ...
    try {
        const sql = `
            ...
            WHERE f.festival_name = 'Venice Film Festival' AND f.festival_year = 2026
        ...
        console.error('Venice Schedule Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch Venice schedule: ${error.message}`,
        })
    }
})
