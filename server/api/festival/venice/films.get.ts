...
export default defineEventHandler(async (event) => {
    ...
    try {
        let sql = `SELECT * FROM festival_films WHERE festival_name = 'Venice Film Festival' AND festival_year = 2026`
        ...
        console.error('Venice Films Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch Venice films: ${error.message || error}`,
        })
    }
})
