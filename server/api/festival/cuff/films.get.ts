...
export default defineEventHandler(async (event) => {
    ...
    try {
        let sql = `SELECT * FROM festival_films WHERE festival_name = 'Calgary Underground Film Festival' AND festival_year = 2026`
        ...
        console.error('CUFF Films Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch CUFF films: ${error.message || error}`,
        })
    }
})
