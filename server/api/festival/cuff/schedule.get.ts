...
export default defineEventHandler(async (event) => {
    ...
    try {
        const sql = `
            ...
            WHERE f.festival_name = 'Calgary Underground Film Festival' AND f.festival_year = 2026
        ...
        console.error('CUFF Schedule Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch CUFF schedule: ${error.message}`,
        })
    }
})
