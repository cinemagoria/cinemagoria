...
export default defineEventHandler(async (event) => {
    ...
    try {
        const sql = `
            ...
            WHERE f.festival_name = 'BAFICI' AND f.festival_year = 2026
        ...
        console.error('BAFICI Schedule Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch BAFICI schedule: ${error.message}`,
        })
    }
})
