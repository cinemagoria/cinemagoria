...
export default defineEventHandler(async (event) => {
    ...
    try {
        const sql = `
            ...
            WHERE f.festival_name = 'Slamdance Film Festival' AND f.festival_year = 2026
        ...
        console.error('Slamdance Schedule Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch Slamdance schedule: ${error.message}`,
        })
    }
})
