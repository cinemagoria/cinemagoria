...
export default defineEventHandler(async (event) => {
    ...
    try {
        const sql = `
            ...
            WHERE f.festival_name = 'FrightFest' AND f.festival_year = 2026
        ...
        console.error('FRIGHTFEST Schedule Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch FRIGHTFEST schedule: ${error.message}`,
        })
    }
})
