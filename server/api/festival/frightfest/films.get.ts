...
export default defineEventHandler(async (event) => {
    ...
    try {
        let sql = `SELECT * FROM festival_films WHERE festival_name = 'FrightFest' AND festival_year = 2026`
        ...
        console.error('FRIGHTFEST Films Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch FRIGHTFEST films: ${error.message || error}`,
        })
    }
})
