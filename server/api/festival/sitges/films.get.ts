...
export default defineEventHandler(async (event) => {
    ...
    try {
        let sql = `SELECT * FROM festival_films WHERE festival_name = 'Sitges Film Festival' AND festival_year = 2026`
        ...
        console.error('Sitges Films Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch Sitges films: ${error.message || error}`,
        })
    }
})
