...
export default defineEventHandler(async (event) => {
    ...
    try {
        const sql = `
            ...
            WHERE f.festival_name = 'Sitges Film Festival' AND f.festival_year = 2026
        ...
        console.error('Sitges Schedule Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch Sitges schedule: ${error.message}`,
        })
    }
})
