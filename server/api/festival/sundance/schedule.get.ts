...
export default defineEventHandler(async (event) => {
    ...
    try {
        const sql = `
            ...
            WHERE f.festival_name = 'Sundance Film Festival' AND f.festival_year = 2026
        ...
        console.error('Sundance Schedule Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch Sundance schedule: ${error.message}`,
        })
    }
})
