...
export default defineEventHandler(async (event) => {
    ...
    try {
        let sql = `SELECT * FROM festival_films WHERE festival_name = 'Tribeca Festival' AND festival_year = 2026`
        ...
        let films = result.rows.map((row: any) => {
            ...
            return {
                ...
                poster_path: (tmdbData.tmdb_poster ? tmdbData.tmdb_poster : (tmdbData.poster_path ? `https://image.tmdb.org/t/p/w500${tmdbData.poster_path}` : null)),
        ...
        console.error('Tribeca Films Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch Tribeca films: ${error.message || error}`,
        })
    }
})
