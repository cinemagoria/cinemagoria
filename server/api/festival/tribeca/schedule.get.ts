...
export default defineEventHandler(async (event) => {
    ...
    try {
        const sql = `
            ...
            WHERE f.festival_name = 'Tribeca Festival' AND f.festival_year = 2026
        ...
        console.error('Tribeca Schedule Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch Tribeca schedule: ${error.message}`,
        })
    }
})
