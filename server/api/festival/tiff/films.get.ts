...
export default defineEventHandler(async (event) => {
    ...
    try {
        let sql = `SELECT * FROM festival_films WHERE festival_name = 'Toronto International Film Festival' AND festival_year = 2026`
        ...
        console.error('TIFF Films Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch TIFF films: ${error.message || error}`,
        })
    }
})
