...
export default defineEventHandler(async (event) => {
    ...
    try {
        const sql = `
            ...
            WHERE f.festival_name = 'Toronto International Film Festival' AND f.festival_year = 2026
        ...
        console.error('TIFF Schedule Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch TIFF schedule: ${error.message}`,
        })
    }
})
