...
export default defineEventHandler(async (event) => {
    ...
    try {
        let sql = `SELECT * FROM festival_films WHERE festival_name = 'Busan International Film Festival' AND festival_year = 2026`
        ...
        console.error('BIFF Films Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch BIFF films: ${error.message || error}`,
        })
    }
})
