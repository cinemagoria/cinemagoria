...
export default defineEventHandler(async (event) => {
    ...
    try {
        const sql = `
            ...
            WHERE f.festival_name = 'Karlovy Vary International Film Festival' AND f.festival_year = 2026
        ...
        console.error('KVIFF Schedule Fetch Error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch KVIFF schedule: ${error.message}`,
        })
    }
})
