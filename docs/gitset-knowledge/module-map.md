...
- `pages/auth-success.vue` — Handles the post-authentication success flow, displaying loading, error, or success states and redirecting the user after a countdown.
- `pages/awards/index.vue` — Displays a comprehensive list of major film awards, allowing users to browse winners and nominees by award and year, with navigation to related movies and media.
- `pages/changelog/index.vue` — Displays a changelog of releases, new features, and fixes for the platform, fetched directly from the GitHub repository releases.
- `pages/contact/index.vue` — Provides a contact form for users to submit inquiries related to their account, watchlist, missing titles, or other platform-related questions.
- `pages/faq/index.vue` — Presents a Frequently Asked Questions (FAQ) page with an interactive table of contents and collapsible sections explaining the platform's framework and infrastructure.
- `pages/festival/bafici-2026/index.vue` — Displays detailed information for the BAFICI 2026 film festival, including films, awards, and schedule, with search and filtering capabilities.
- `pages/festival/berlinale-2026/index.vue` — Displays detailed information for the Berlinale 2026 film festival, including films, awards, and schedule, with search and filtering capabilities.
- `pages/festival/bifff-2026/index.vue` — Displays detailed information for the BIFFF 2026 film festival, including films, awards, and schedule, with search and filtering capabilities.
- `pages/festival/cannes-2026/index.vue` — Displays detailed information for the Cannes Film Festival 2026, including films, awards, and schedule, with search and filtering capabilities.
...
- `pages/festival/sundance-2026/index.vue` — Vue page component for displaying details, films, awards, and schedule for the Sundance Film Festival 2026, including search and mobile responsiveness.
- `pages/festival/sxsw-2026/index.vue` — Vue page component for displaying details, films, awards, and schedule for the SXSW Film Festival 2026, including search and mobile responsiveness.
- `pages/festival/tiff-2026/index.vue` — Vue page component for displaying details, films, awards, and schedule for the TIFF 2026 festival, including search and mobile responsiveness.
- `pages/festival/tribeca-2026/index.vue` — Vue page component for displaying details, films, awards, and schedule for the Tribeca Film Festival 2026, including search and mobile responsiveness.
- `pages/festival/venice-2026/index.vue` — Vue page component for displaying details, films, awards, and schedule for the Venice Film Festival 2026, including search and mobile responsiveness.
...
- `pages/genre/[id]/tv.vue` — Vue page component that displays a paginated list of TV shows filtered by a specific genre ID, fetching data from an API and handling 'load more' functionality.
- `pages/index.vue` — Vue page component for the homepage, featuring various carousels for festivals, news, trending movies/TV, and conditional banners for live events like Oscars and Cannes 2026.
- `pages/lists/[slug].vue` — Displays a specific user-created list of movies and TV shows, allowing the owner to rename, edit, filter, and sort its contents.
...
- `pages/login/index.vue` — Serves as the entry point for user authentication, displaying the AuthModal component with the login tab pre-selected.
...
- `pages/movie/category/[name].vue` — Displays a paginated list of movies belonging to a specific category (e.g., trending, popular), with a 'load more' feature.
...
- `pages/news/index.vue` — Displays a feed of the latest news articles, offering filtering by source and category, and search functionality.
- `pages/noir/index.vue` — Displays the N.O.I.R Archive, a curated list of historical titles, with sorting options and functionality to clone the archive into a user's personal list.
...
- `pages/production/[slug].vue` — Vue page component for displaying a specific production company's details, including its movies and TV shows, with filtering and sorting options.
...
- `pages/register/index.vue` — Vue page component that serves as an entry point for user registration, displaying the AuthModal with the register tab active upon mounting.
- `pages/search/index.vue` — Vue page component for displaying search results across movies, TV shows, and people, with loading indicators and infinite scrolling.
- `pages/settings/index.vue` — Vue page component for managing user account settings, including profile information, avatar, alias, and privacy preferences.
...
- `pages/streaming/[slug].vue` — Displays a streaming platform's details, including movies and TV shows, with filtering and sorting options.
- `pages/streaming/followed.vue` — Displays followed streaming content, differentiating between movies and TV shows based on the URL query parameter.
- `pages/tv/[id].vue` — Displays detailed information for a specific TV show, including overview, cast, episodes, videos, images, soundtracks, and awards.
- `pages/tv/category/[name].vue` — Displays a list of TV shows belonging to a specific category (e.g., 'trending', 'popular'), with pagination.
- `pages/tv/followed.vue` — Displays TV shows that the user is following.
- `pages/tv/index.vue` — Serves as the main discovery page for TV shows, featuring popular, top-rated, on-air, and airing today series, with filtering options.
...
- `public/sw.js` — A self-destroying service worker script designed to unregister itself and reload all open client pages upon activation, ensuring no service worker remains active.
  - This file is explicitly marked as not for version control.
