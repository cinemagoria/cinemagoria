...
export function useDiscoverQuery(type) {
  ...
  const sortLabel = computed(() => DISCOVER_SORTS.find(item => item.value === sort.value)?.label || 'Mejor valoradas (IMDb)');
  ...
  const minVotesLabel = computed(() => MIN_VOTE_OPTIONS.find(item => item.value === minVotes.value)?.label || `${minVotes.value}+ votos`);
  const formatLabel = computed(() => FORMAT_OPTIONS.find(item => item.value === format.value)?.label || 'Largometrajes');
  ...
  const chips = computed(() => {
    ...
    if (yearFrom.value) list.push({ key: 'yearFrom', label: `Desde ${yearFrom.value}` });
    if (yearTo.value) list.push({ key: 'yearTo', label: `Hasta ${yearTo.value}` });
    if (ratingMin.value) list.push({ key: 'ratingMin', label: `Puntuación ≥ ${ratingMin.value}` });
    if (ratingMax.value) list.push({ key: 'ratingMax', label: `Puntuación ≤ ${ratingMax.value}` });
    return list;
  });
