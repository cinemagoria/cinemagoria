    ...
    <div class="container header-container">
      <div class="festival-hero">
        ...
        <a href="https://www.calgaryundergroundfilm.org" target="_blank" class="hero-backdrop">
            <img
              ref="backdropImgRef"
              src="/festivals/cuff/cuff_backdrop_2026_eng.webp"
              alt="CUFF Backdrop"
      ...
      <!-- Winners Showcase: visible solo si el festival ya terminó y hay awards cargados -->
      <WinnersCarousel
        v-if="awards.length > 0"
        :awards="awards"
        :year="2026"
        :stat="{ main: '15,511 attendees', sub: '+25% vs 2025 · Record edition' }"
      />
              ...
              <transition :name="slideDirection" mode="out-in">
                <div class="carousel-card" :key="infoSlide">
                  ...
                  <template v-if="infoSlide === 0">
                    ...
                    <p class="carousel-desc"><strong>Website:</strong> <a href="https://www.calgaryundergroundfilm.org" target="_blank" class="accent-link">calgaryundergroundfilm.org</a></p>
                    <p class="carousel-desc"><strong>Dates:</strong> April 16 – 26, 2026</p>
                    <p class="carousel-desc">CUFF (Calgary Underground Film Festival) celebrates bold, independent and genre cinema — from cult horror and comedy to documentary, animation and experimental works. Features include opening / closing night galas, anniversary screenings and curated shorts packages.</p>
                  ...
                  <template v-if="infoSlide === 1">
                    ...
                    <ul class="price-list">
                      <li><span class="price-label">Single Ticket (regular)</span><span class="price-value">$15</span></li>
                      <li><span class="price-label">Opening / Closing Night</span><span class="price-value">$20</span></li>
                      <li><span class="price-label">Festival Pass (all access)</span><span class="price-value">$200</span></li>
                      <li><span class="price-label">Hard Core Logo (Free Screening)</span><span class="price-value">Free</span></li>
                    </ul>
                    <p class="carousel-desc">Tickets available at the Globe Cinema box office and online via the official CUFF site.</p>
                  </template>
                  ...
                  <template v-if="infoSlide === 2">
                    ...
                    <p class="carousel-desc">All screenings take place at the historic Globe Cinema in downtown Calgary.</p>
                  </template>
                </div>
              </transition>
            </div>
...
onMounted(async () => {
    try {
        const [filmsData, scheduleData, awardsData] = await Promise.all([
            $fetch('/api/festival/cuff/films?limit=200&sort=title'),
            $fetch('/api/festival/cuff/schedule').catch(() => ({ results: [] })),
            $fetch('/api/festival/cuff/awards').catch(() => ({ results: [] })),
        ]);
