  ...
  <main class="main">
    <div class="container header-container">
      <div class="festival-hero">
        ...
        <a href="https://www.calgaryundergroundfilm.org" target="_blank" class="hero-backdrop">
            <img
              src="/festivals/cuff/cuff_backdrop_2026_es.webp"
              alt="CUFF Backdrop"
            />
              ...
              <transition :name="slideDirection" mode="out-in">
                <div class="carousel-card" :key="infoSlide">
                  ...
                  <template v-if="infoSlide === 0">
                    ...
                    <p class="carousel-desc"><strong>Sitio web:</strong> <a href="https://www.calgaryundergroundfilm.org" target="_blank" class="accent-link">calgaryundergroundfilm.org</a></p>
                    <p class="carousel-desc"><strong>Fechas:</strong> 16 – 26 de Abril, 2026</p>
                    <p class="carousel-desc">CUFF (Calgary Underground Film Festival) celebra el cine independiente y de género — desde el terror y la comedia de culto hasta el documental, la animación y obras experimentales. Incluye galas de apertura / clausura, proyecciones aniversario y paquetes curados de cortometrajes.</p>
                    <p class="carousel-desc"><strong>Social:</strong> @calgaryundergroundfilmfest (Instagram)</p>
                  </template>

                  <!-- Slide 1: Acceso y Entradas -->
                  <template v-if="infoSlide === 1">
                    ...
                    <ul class="price-list">
                      <li><span class="price-label">Entrada individual (regular)</span><span class="price-value">$15</span></li>
                      <li><span class="price-label">Apertura / Clausura</span><span class="price-value">$20</span></li>
                      <li><span class="price-label">Pase Festival (acceso completo)</span><span class="price-value">$200</span></li>
                      <li><span class="price-label">Hard Core Logo (Función Gratuita)</span><span class="price-value">Gratis</span></li>
                    </ul>
                    <p class="carousel-desc">Entradas disponibles en la boletería del Globe Cinema y online en el sitio oficial de CUFF.</p>
                  </template>
                  <template v-if="infoSlide === 2">
                    ...
                    <div class="venue-list">
                      <div class="venue-item"><strong>Globe Cinema</strong><span>617 8 Ave SW, Calgary, AB T2P 1H1</span></div>
                    </div>
                    <p class="carousel-desc">Todas las funciones se realizan en el histórico Globe Cinema en el centro de Calgary.</p>
                  </template>
                </div>
              </transition>
            </div>
...
const normalizeText = (str) => { if (!str) return ''; return String(str).toLowerCase().normalize('NFD').replace(/\p{Diacritic}/gu, ''); };
const scheduleSearchActiveQuery = computed(() => { if (!isScheduleSearchActive.value) return ''; return scheduleSearch.value.trim(); });
const toggleScheduleSearch = () => { isScheduleSearchActive.value = !isScheduleSearchActive.value; if (!isScheduleSearchActive.value) scheduleSearch.value = ''; else nextTick(() => scheduleSearchInput.value?.focus()); };
...
const loading = ref(true); const films = ref({ results: [] }); const awards = ref([]); const schedule = ref([]); const openDays = ref(new Set());
...
watch([loading, activeTab, selectionSections], () => { if (!loading.value && activeTab.value === 'films') nextTick(() => setupSectionObserver()); else if (sectionObserver) sectionObserver.disconnect(); }, { flush: 'post' });
const formatDate = (dateStr) => { const options = { weekday: 'long', month: 'long', day: 'numeric' }; const [year, month, day] = dateStr.split('-').map(Number); const date = new Date(year, month - 1, day).toLocaleDateString('es-ES', options); return date.charAt(0).toUpperCase() + date.slice(1); };
const formatTime = (timeStr) => new Date(timeStr).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Edmonton' });
const filteredSchedule = computed(() => { const q = scheduleSearchActiveQuery.value; if (!q) return schedule.value; const needle = normalizeText(q); return schedule.value.filter(s => normalizeText(s.film?.title).includes(needle) || normalizeText(s.film?.director).includes(needle)); });
const groupedScreenings = computed(() => { const groups = {}; (filteredSchedule.value || []).forEach(s => { const dateKey = s.start_time.split('T')[0]; if (!groups[dateKey]) groups[dateKey] = []; groups[dateKey].push(s); }); return Object.keys(groups).sort().reduce((obj, key) => { obj[key] = groups[key]; return obj; }, {}); });
const toggleDay = (date) => { if (openDays.value.has(date)) openDays.value.delete(date); else openDays.value.add(date); };
const isOpen = (date) => scheduleSearchActiveQuery.value ? true : openDays.value.has(date);
...
const scrollToDay = (date) => { activeDay.value = date; const root = scheduleContentRef.value; if (!root) return; for (const el of root.querySelectorAll('[data-day]')) if (el.getAttribute('data-day') === date) { el.scrollIntoView({ behavior: 'smooth', block: 'start' }); break; } };
const setupDayObserver = () => { if (typeof window === 'undefined' || typeof IntersectionObserver === 'undefined') return; if (dayObserver) dayObserver.disconnect(); const root = scheduleContentRef.value; if (!root) return; const els = root.querySelectorAll('[data-day]'); if (!els.length) return; dayObserver = new IntersectionObserver((entries) => { for (const entry of entries) if (entry.isIntersecting) activeDay.value = entry.target.getAttribute('data-day'); }, { rootMargin: '-110px 0px -70% 0px', threshold: 0 }); els.forEach((el) => dayObserver.observe(el)); };
watch(scheduleDays, (days) => { if (days.length && !days.some((d) => d.date === activeDay.value)) activeDay.value = days[0]?.date || ''; }, { immediate: true });
watch([loading, activeTab, scheduleDays], () => { if (!loading.value && activeTab.value === 'schedule') nextTick(() => setupDayObserver()); else if (dayObserver) dayObserver.disconnect(); }, { flush: 'post' });
const updateIsMobile = (e) => { isMobile.value = e.matches; };
onMounted(() => { if (typeof window !== 'undefined' && window.matchMedia) { mobileMql = window.matchMedia('(max-width: 900px)'); isMobile.value = mobileMql.matches; mobileMql.addEventListener('change', updateIsMobile); } });
onBeforeUnmount(() => { if (sectionObserver) sectionObserver.disconnect(); if (dayObserver) dayObserver.disconnect(); if (mobileMql) mobileMql.removeEventListener('change', updateIsMobile); });
onMounted(async () => { try { const [filmsData, scheduleData, awardsData] = await Promise.all([$fetch('/api/festival/cuff/films?limit=500&sort=title'), $fetch('/api/festival/cuff/schedule'), $fetch('/api/festival/cuff/awards').catch(() => ({ results: [] }))]); films.value = filmsData; awards.value = awardsData.results || []; schedule.value = scheduleData.results || []; if (schedule.value.length > 0) { const dates = new Set(schedule.value.map(s => s.start_time.split('T')[0])); dates.forEach(d => openDays.value.add(d)); } } catch (e) { console.error('Error fetching CUFF festival data', e); } finally { loading.value = false; } });
</script>
