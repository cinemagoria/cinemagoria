  ...
  <main class="main">
    <div class="container header-container">
      <div class="festival-hero">
        ...
        <a href="https://tiff.net/ticket-pricing" target="_blank" rel="noopener noreferrer" class="hero-backdrop">
            <img
              ref="backdropImgRef"
              src="/festivals/tiff/tiff_backdrop_2026_es.webp"
              alt="Backdrop de TIFF 2026"
        ...
        <div v-if="activeTab === 'films'" class="selection">
          ...
          <div v-if="orderedCategories.length === 0" class="rollout-banner">
            ...
            <div class="rollout-banner__copy">
              <h3>Actualizaciones del line-up en curso</h3>
              <p>El line-up de este festival se está actualizando. Se irán sumando películas y pantallas progresivamente a medida que se anuncien y verifiquen.</p>
            </div>
          </div>
          <div v-else class="selection-layout">
            <aside class="selection-nav" aria-label="Ir a la sección">
        ...
        <div v-else-if="activeTab === 'schedule'" class="schedule-container">
          <div class="schedule-toolbar" :class="{ 'search-active': isScheduleSearchActive }">
            ...
            <div class="search-wrapper" :class="{ 'active': isScheduleSearchActive }">
              <button class="search-toggle-btn" @click="toggleScheduleSearch" :class="{ 'active': isScheduleSearchActive }" aria-label="Buscar en el horario">
          ...
          <div v-if="scheduleDays.length" class="schedule-layout">
            <aside class="schedule-nav" aria-label="Ir al día">
                ...
                <div v-show="!isMobile || isOpen(date)" class="screenings-list">
                  <div v-for="screening in dayScreenings" :key="screening.id" class="screening-card">
                     <div class="time-block">
                      ...
                      <div class="film-info">
                         <a
                            :href="screening.film.source_url || 'https://tiff.net'"
                            target="_blank"
              ...
              <transition :name="slideDirection" mode="out-in">
                <div class="carousel-card" :key="infoSlide">
                  ...
                  <template v-if="infoSlide === 0">
                    ...
                    <p class="carousel-desc">El <strong>Festival Internacional de Cine de Toronto (TIFF)</strong> es el festival de cine de asistencia pública más grande del mundo y una de las plataformas de lanzamiento más importantes rumbo a los premios de la temporada. La <strong>51ª edición</strong>, presentada por Rogers, abre con el estreno mundial de <em>Being Heumann</em>, de Siân Heder, en el Roy Thomson Hall.</p>
                    <p class="carousel-desc">Este año también marca el primer <strong>TIFF: The Market</strong>, del 10 al 16 de septiembre en el Metro Toronto Convention Centre.</p>
                  </template>
                  ...
                  <template v-if="infoSlide === 1">
                    ...
                    <ul class="price-list">
                      <li><span class="price-label">Entradas individuales</span><span class="price-value">USD 25–60</span></li>
                      <li><span class="price-label">Paquetes curados</span><span class="price-value">USD 200–800</span></li>
                      <li><span class="price-label">Pases de festival</span><span class="price-value">USD 400–1.500+</span></li>
                    </ul>
                    <p class="carousel-desc">Las entradas individuales salen a la venta para miembros de TIFF desde el 21 de agosto (según nivel de membresía), y para el público general el 31 de agosto de 2026, a través de Ticketmaster.</p>
...
// independent parallel sidebars like Cannes/Venice — so this stays a flat
// list, same pattern as FrightFest. OTHER stays as a safety net.
...
const formatTime = (timeStr) => {
    // These are in-person Toronto screenings — always show the venue-local
...
// TIFF's Primetime strand is TV (Alice, Carrie, Crystal Lake, etc.), stored
// in festival_films as movie-shaped rows enriched from TMDB's /tv/ endpoint
// (see #415). The stored tmdb_data already carries media_type: 'tv', which
// survives the API's tmdbData spread — but we force it here too, keyed off
// the reliable category column, so Card routing to /tv/{id} instead of
// /movie/{id} never depends on the JSON blob alone. Scoped to this page only;
// no shared component or other festival's page is touched.
function withPrimetimeAsTv(filmsData) {
    const results = (filmsData?.results || []).map((f) => {
        const key = String(f.category || f.section || '').trim();
        return key === 'PRIMETIME' ? { ...f, media_type: 'tv' } : f;
    });
...
onMounted(async () => {
    try {
        ...
        films.value = withPrimetimeAsTv(filmsData);
