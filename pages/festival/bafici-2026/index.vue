...
const formatTime = (timeStr) => new Date(timeStr).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Argentina/Buenos_Aires' });
...
onMounted(async () => { try { const [filmsData, scheduleData, awardsData] = await Promise.all([$fetch('/api/festival/bafici/films?limit=500&sort=title'), $fetch('/api/festival/bafici/schedule'), $fetch('/api/festival/bafici/awards').catch(() => ({ results: [] }))]); films.value = filmsData; awards.value = awardsData.results || []; schedule.value = scheduleData.results || []; if (schedule.value.length > 0) { const dates = new Set(schedule.value.map(s => s.start_time.split('T')[0])); dates.forEach(d => openDays.value.add(d)); } } catch (e) { console.error('Error fetching BAFICI festival data', e); } finally { loading.value = false; } });
</script>
