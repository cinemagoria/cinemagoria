      ...
      <div v-if="isLoggedIn" class="notifications-container">
          ...
          <div class="header-actions">
            <div class="header-actions-left">
              ...
              <div v-if="viewMode === 'notifications'" class="filter-switch-three">
                <button :class="['filter-btn', { active: filterMode === 'unread' }]" @click="setFilter('unread')">Unread</button>
                <button :class="['filter-btn', { active: filterMode === 'all' }]" @click="setFilter('all')">All</button>
              </div>
            </div>
        ...
        <div v-else-if="viewMode === 'activity'" class="notifications-list activity-feed">
          ...
          <div v-else-if="activityItems.length === 0" class="no-notifications">
            ...
            <p>No activity yet from users you follow.</p>
            <p class="sub-text">Follow other users to see their ratings and lists here.</p>
          </div>
          <template v-else>
                ...
                <div class="notification-text">
                  <div class="notification-title">
                    ...
                    <span class="has-release">
                      {{ item.activity_type === 'rating' ? ' rated' : item.activity_type === 'review' ? ' reviewed' : ' created a list' }}
                    </span>
                  </div>
                  <div v-if="item.item_name" class="notification-media" style="cursor:pointer;" @click="navigateToItem(item)">
                    {{ item.item_name }}
                    <span v-if="item.rating" style="color:#8BE9FD;font-weight:600;"> · {{ item.rating }}/10</span>
                  </div>
            ...
            <div v-if="activityHasMore" style="text-align:center;margin-top:1.5rem;">
              <button class="mark-all-button" @click="loadMoreActivity">Load More</button>
            </div>
          </template>
        </div>
