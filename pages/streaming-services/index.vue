<template>
  <main class="sv-page">
    ...
    <header class="page-header sv-header">
      <span class="sv-eyebrow">Catalogue</span>
      <h1 class="page-title">Streaming Services</h1>
      <p class="page-subtitle">
        Every service tracked across the catalogue. Search by name or jump straight to a letter.
      </p>
    </header>

    <div class="sv-gutter">
      <section class="sv-panel" aria-label="Filter streaming services">
        <div class="sv-search">
          ...
          <button v-if="query" type="button" class="sv-search__clear" aria-label="Clear search" @click="query = ''">
        ...
        <div class="sv-alpha" role="group" aria-label="Filter by initial">
          <button
            type="button"
            class="sv-alpha__key"
    ...
    <section v-if="showPopular" class="sv-gutter sv-zone">
      <div class="rule">
        <span class="rule__text">Most popular</span>
    ...
    <section class="sv-gutter sv-zone sv-zone--last">
      <div class="rule">
        ...
        <button v-if="hasFilters" type="button" class="sv-reset" @click="resetFilters">Reset</button>
      </div>
      ...
      <p v-else class="sv-empty">
        No streaming service matches <strong>{{ query || letter }}</strong>.
      </p>

      <div v-if="hasMore" class="sv-more">
        <button type="button" class="sv-more__btn" @click="page += 1">Load more</button>
        <span class="sv-more__meta">{{ visible.length.toLocaleString('en-US') }} of {{ filtered.length.toLocaleString('en-US') }}</span>
      </div>
    </section>
  </main>
</template>
