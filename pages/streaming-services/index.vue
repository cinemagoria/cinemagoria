  ...
  <div class="page-container">
    <div class="listing">
      <nav class="navbar navbar-welcome">
        <h1 class="title-primary page-title">Plataformas de Streaming</h1>
        <h2 class="title-secondary page-subtitle">
          Descubre las principales plataformas de streaming disponibles
        </h2>
      </nav>
        ...
        >
          <div class="logo-wrapper">
            <img 
              ...
              :src="`${apiImgUrl}/w500${provider.logo_path}`" 
              :alt="provider.name" 
              class="company-logo"
              loading="lazy"
            >
            <span v-else class="company-name-fallback">{{ provider.name }}</span>
          </div>
          <div class="card-footer">
            <span class="card-label">{{ provider.name }}</span>
          </div>
        </NuxtLink>
      </div>
    </div>
...
const sortedProviders = computed(() => {
  return STREAMING_PROVIDERS.sort((a, b) => 
