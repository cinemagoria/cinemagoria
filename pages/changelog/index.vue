<template>
  <main class="page">
    ...
    <header class="hero">
      <div class="heroInner">
        <span class="eyebrow">Notas de Versión</span>
        <h1 class="heroTitle">Changelog</h1>
        <p class="heroLead">
          Un registro continuo de versiones, nuevas funciones, correcciones y mejoras de la plataforma &mdash; tomado directamente del repositorio de origen.
        </p>
      </div>
    </header>
    ...
    <div class="layout">
      ...
      <div v-else-if="error" class="errorCard">
        <p>{{ error }}</p>
        <button @click="fetchReleases" class="primaryBtn">Reintentar</button>
      </div>
      ...
      <div v-else class="releaseList">
        <article v-for="release in releases" :key="release.id" class="releaseCard">
          ...
          <button @click="toggleExpand(release.id)" class="expandBtn">
            {{ expanded[release.id] ? 'Mostrar menos' : 'Leer más' }}
          </button>
        </article>
...
export default {
  head() {
    return {
      title: 'Changelog — Notas de Versión y Actualizaciones',
      meta: [
        { hid: 'description', name: 'description', content: 'Changelog — últimas versiones, nuevas funciones, correcciones y mejoras de la plataforma.' },
  ...
  methods: {
    ...
    async fetchReleases() {
      ...
      try {
        ...
        this.error = 'No se pudo cargar el changelog. Probá de nuevo en un momento.';
      } finally {
        this.loading = false;
      }
    },
