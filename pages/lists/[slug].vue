          ...
          <div class="list-meta">
              ...
              <div v-if="isOwner" class="privacy-wrapper" style="position: relative;">
                  <button @click.stop="togglePrivacyDropdown" class="privacy-btn">
                      ...
                      <svg width="18" height="18" fill="currentColor" style="margin-left:2px" viewBox="0 0 24 24"><path d="m7 10 5 5 5-5z"/></svg>
                  </button>
               ...
               <button v-if="isOwner" @click="openRenameModal" class="share-btn-icon" aria-label="Rename List" style="margin-right: 5px;">
                   <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" stroke="#8be9fd" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" style="display:block;min-width:20px" viewBox="0 0 24 24"><path d="M12 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.375 2.625a1 1 0 0 1 3 3l-9.013 9.014a2 2 0 0 1-.853.505l-2.873.84a.5.5 0 0 1-.62-.62l.84-2.873a2 2 0 0 1 .506-.852z"/></svg>
              </button>
              
               <button v-if="!isOwner && list.is_public" @click="handleCloneList" class="share-btn-icon" aria-label="Clonar Lista" title="Clonar a mi perfil">
               ...
               <button v-if="list.is_public" @click="openShareModal" class="share-btn-icon" aria-label="Share">
                 <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" stroke="#8be9fd" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" style="display:block;min-width:20px" viewBox="0 0 24 24"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><path d="m8.59 13.51 6.83 3.98m-.01-10.98-6.82 3.98"/></svg>
              </button>
          </div>
      </div>
    </div>
        ...
        <transition name="slide-up">
           <div v-if="undoItem" class="undo-banner">
             <div class="undo-content">
                 <span>Eliminando "{{ undoItem.details.nameForDb }}"</span>
             ...
             <div class="new-controls-container">
              ...
              <div class="action-buttons">
                <button class="control-btn" @click="openFiltersModal">
                  <svg width="22" height="22" fill="currentColor" viewBox="0 0 24 24"><path d="M3 17v2h6v-2zM3 5v2h10V5zm10 16v-2h8v-2h-8v-2h-2v6zM7 9v2H3v2h4v2h2V9zm14 4v-2H11v2zm-6-4h2V7h4V5h-4V3h-2z"/></svg>
                </button>
              </div>
            </div>
                      ...
                      <transition name="fade">
                         <div v-if="activeCardMenuId === item.details.idForDb" class="action-dropdown" @click.stop>
                             ...
                             <div class="dropdown-item" @click="openAddModal(item); activeCardMenuId = null">
                                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" style="margin-right:6px" viewBox="0 0 24 24"><path d="M12 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.375 2.625a1 1 0 0 1 3 3l-9.013 9.014a2 2 0 0 1-.853.505l-2.873.84a.5.5 0 0 1-.62-.62l.84-2.873a2 2 0 0 1 .506-.852z"/></svg>
                                Edit...
                             </div>
          ...
          <div class="rating-modal">
            ...
            <div class="rating-content">
              ...
              <div class="review-section">
                <textarea
                  v-model="userReview"
                  :placeholder="selectedRating > 0 ? ratingDescriptions[selectedRating - 1] : 'Selecciona una calificación primero'"
        ...
        <div class="filters-content">
          ...
          <div class="filter-group" v-if="isOwner">
            ...
            <select v-model="selectedUserRating" class="filter-input">
              ...
              <option value="not-rated">No Calificado</option>
    ...
    <div v-if="shareModalVisible" class="modal-overlay">
        <div class="share-modal-content">
          <div class="share-modal-header">
            <h2>Compartir Esta Colección</h2>
        ...
        <div class="create-list-modal-content" @click.stop>
             ...
             <div class="modal-body-cl">
                 <div class="form-group-cl">
                    ...
                    <input type="text" v-model="newListName" class="input-cl" @keyup.enter="updateListName" autofocus placeholder="ej. Mejores Películas">
                 </div>
                 <div class="form-group-cl" style="margin-top: 15px;">
                    <label>Descripción (Opcional)</label>
                    <textarea v-model="newListDescription" class="input-cl textarea-cl" placeholder="Agregar una descripción..."></textarea>
                 </div>
                 <div class="actions-cl">
                      <button @click="closeRenameModal" class="cancel-btn-cl">Cancelar</button>
                      <button @click="updateListName" class="create-btn-cl" :disabled="!newListName || !newListName.trim()">Guardar</button>
                 </div>
             </div>
        </div>
    </div>
...
export default {
        ...
        async handleCloneList() {
             ...
             if (!userEmail) {
                 if (typeof window !== 'undefined') {
                     ...
                     alert('Debes iniciar sesión para clonar listas.');
                 }
                 return;
             }
    ...
    async saveRatingAndReview() {
      if (this.selectedRating === 0) {
        alert('Por favor selecciona una calificación entre 1 y 10');
        return;
      }
      ...
      try {
        ...
        if (!response.ok) {
             throw new Error('Error updating rating. Ensure item is in your library.');
        }
        ...
        alert('No se pudo guardar la calificación. Por favor intenta de nuevo.');
      }
    },
