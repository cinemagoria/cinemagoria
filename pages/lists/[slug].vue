  ...
  <div class="list-page">
    ...
    <template v-else>
      ...
      <header class="list-hero">
        <div class="title-row">
          ...
          <button v-if="isOwner" @click="openRenameModal" class="title-edit-btn" aria-label="Renombrar lista" title="Renombrar">
        ...
        <div class="list-meta">
          <span class="meta-author">{{ isOwner ? 'por Tí' : 'por ' + (list.owner_name || 'Desconocido') }}</span>
          <span class="meta-dot">·</span>
          <span class="meta-count">{{ items.length }} {{ items.length === 1 ? 'elemento' : 'elementos' }}</span>
        </div>
        ...
        <div v-if="items.length === 0" class="list-actions">
          <div v-if="isOwner" class="privacy-wrapper">
            ...
            <transition name="fade">
              <div v-if="privacyDropdownOpen" class="privacy-dropdown">
                <div class="dropdown-item" @click="setPrivacy(true)" :class="{ active: list.is_public }">Pública</div>
                <div class="dropdown-item" @click="setPrivacy(false)" :class="{ active: !list.is_public }">Privada</div>
              </div>
            </transition>
          </div>
          <button v-if="!isOwner && list.is_public" @click="handleCloneList" class="action-pill" aria-label="Clonar lista" title="Clonar a mi perfil">
          ...
          <button v-if="list.is_public" @click="openShareModal" class="action-pill" aria-label="Compartir lista">
      ...
      <transition name="slide-up">
        <div v-if="undoItem" class="undo-banner">
          <span>Eliminado &ldquo;{{ undoItem.details.nameForDb }}&rdquo;</span>
      ...
      <div v-if="items.length === 0 && !undoItem" class="state-card">
        ...
        <p>Explorá y agregá películas o series.</p>
        <nuxt-link to="/" class="primary-btn">Explorar contenido</nuxt-link>
      </div>
      ...
      <section v-else class="list-panel">
        ...
        <div class="panel-toolbar">
          ...
          <div class="toolbar-right">
            ...
            <button class="filter-btn" :class="{ active: hasActiveFilters }" @click="openFiltersModal" aria-label="Filtros y orden">
        ...
        <div v-if="displayItems.length === 0" class="state-card inset">
          <template v-if="filteredItems.length === 0 && hasActiveFilters">
            ...
            <button @click="clearAllFilters" class="primary-btn">Limpiar filtros</button>
          </template>
          <template v-else>
            <img src="/ui/cinema-popcorn.svg" alt="" class="state-icon">
            <h3>Sin {{ filter === 'movies' ? 'películas' : 'series' }}</h3>
            <p>Cambiá de pestaña o agregá contenido.</p>
          </template>
        </div>
        ...
        <template v-else>
          ...
          <div v-if="filteredItems.length > itemsPerPage" class="list-pager">
            <button @click="goToFirst" :disabled="currentPage === 1" aria-label="Primera página">
            ...
            <button @click="goToLast" :disabled="currentPage === totalPages" aria-label="Última página">
            ...
            <div v-for="vm in displayItems" :key="vm.type + '-' + vm.id" class="movie-card" :id="'list-item-' + vm.id">
              <div class="poster-container">
                ...
                <div v-if="isOwner && vm.userRating" class="user-rating-badge"
                  ...
                  :title="vm.hasReview ? 'Tiene reseña' : 'Tu calificación'">
                  {{ vm.userRating }}
                ...
                <div v-if="isOwner" class="card-actions-menu" :class="{ 'menu-open': activeCardMenuId === vm.id }">
                  <button class="dropdown-trigger" @click.prevent.stop="toggleCardMenu(vm.id)" aria-label="Acciones">
              ...
              <div class="card-info">
                ...
                <div class="card-rating">
                  ...
                  <span v-else class="rating-empty">Sin calificar</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-if="filteredItems.length > itemsPerPage" class="list-pager">
            ...
            <span class="pager-info">
              <label for="pageBottom" class="pager-label">Página</label>
    ...
    <div v-if="ratingModalVisible" class="modal-overlay" @click.self="closeRatingModal">
      <div class="glass-modal rating-modal">
        <button class="modal-close" @click="closeRatingModal" aria-label="Cerrar">
        ...
        <h3 class="modal-title">Calificar &lsquo;{{ currentRatingItem?.details?.nameForDb }}&rsquo;</h3>

        <div class="rate-score">
          <span class="rate-score-value">{{ hoverRating || selectedRating || '–' }}</span>
          <span class="rate-score-label">{{ (hoverRating || selectedRating) ? 'sobre 10' : 'Elegí una nota' }}</span>
        </div>
        ...
        <div v-if="isRatingMovie" class="pg-section">
          ...
          <div class="pg-row">
            ...
            <div class="pg-controls">
              ...
              <input v-else type="range" class="pg-slider" min="0" max="100" step="1" v-model.number="progressPercentage" aria-label="Porcentaje visto" />

              <div v-if="ratingRuntime" class="pg-times">
                <div class="pg-time">
                  <span class="pg-time-label">Visto</span>
                  <span class="pg-entry">
                    <input type="number" min="0" :max="Math.floor(ratingRuntime / 60)" v-model.number="watchedHours" aria-label="Horas vistas" />
                    <em>h</em>
                ...
                <div class="pg-time pg-time--right">
                  <span class="pg-time-label">Restante</span>
                  <span class="pg-time-value">{{ progressRemaining }}</span>
                </div>
              </div>
              <p v-else class="pg-no-duration">Duración no disponible</p>
            </div>
          </div>
        </div>
        ...
        <div class="review-section">
          <textarea v-model="userReview"
            :placeholder="selectedRating > 0 ? 'Añadí una reseña opcional…' : 'Seleccioná una calificación'"
        ...
        <div class="modal-actions">
          <button v-if="currentRatingItem && currentRatingItem.details.userRatingForDb && currentRatingItem.details.userRatingForDb !== '-'"
            @click="removeRating" class="btn-ghost danger">Quitar calificación</button>
          <button @click="saveRatingAndReview" class="btn-primary" :disabled="selectedRating === 0">Guardar</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="filtersModalVisible" class="modal-overlay" @click.self="closeFiltersModal">
      <div class="glass-modal filters-modal">
        <button class="modal-close" @click="closeFiltersModal" aria-label="Cerrar">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M6 18 18 6M6 6l12 12"/></svg>
        </button>
        <h3 class="modal-title">Filtros y orden</h3>
        ...
        <div class="filters-content">
          <div class="filter-group">
            ...
            <div class="custom-select" @click="toggleGenreDropdown">
              ...
              <div v-if="genreDropdownOpen" class="dropdown-options">
                <div class="dropdown-option" :class="{ selected: selectedGenre === '' }" @click.stop="selectGenre('')">Todos los géneros</div>
          ...
          <div class="filter-group">
            ...
            <div class="range-inputs">
              <input type="number" v-model.number="customYearStart" :min="1880" :max="currentYear" placeholder="Desde" class="field-input">
              <span class="range-sep">–</span>
              <input type="number" v-model.number="customYearEnd" :min="1880" :max="currentYear" placeholder="Hasta" class="field-input">
            </div>
          ...
          <div class="filter-group">
            ...
            <div class="range-inputs">
              <input type="number" v-model.number="minImdbRating" min="0" max="10" placeholder="Mín" class="field-input">
              <span class="range-sep">–</span>
              <input type="number" v-model.number="maxImdbRating" min="0" max="10" placeholder="Máx" class="field-input">
            </div>
          </div>
          ...
          <div class="filter-group">
            ...
            <div class="range-inputs">
              <input type="number" v-model.number="minImdbVotes" min="0" placeholder="Mín" class="field-input">
              <span class="range-sep">–</span>
              <input type="number" v-model.number="maxImdbVotes" min="0" placeholder="Máx" class="field-input">
            </div>
          </div>
          ...
          <div class="filter-group" v-if="isOwner">
            ...
            <select v-model="selectedUserRating" class="field-input native-select">
              ...
              <option value="not-rated">Sin calificar</option>
          ...
          <div class="modal-actions">
            <button @click="clearAllFilters" class="btn-ghost">Limpiar</button>
            <button @click="closeFiltersModal" class="btn-primary">Aplicar</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="shareModalVisible" class="modal-overlay" @click.self="closeShareModal">
      <div class="glass-modal share-modal">
        <button class="modal-close" @click="closeShareModal" aria-label="Cerrar">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M6 18 18 6M6 6l12 12"/></svg>
        </button>
        <h3 class="modal-title">Compartir esta colección</h3>

        <div class="field-group">
          <label class="filter-label">Enlace</label>
          <div class="link-row">
        ...
        <div class="field-group">
          <label class="filter-label">Mensaje</label>
          <textarea v-model="customMessage" class="field-textarea"></textarea>
        </div>

        <div class="field-group">
          <label class="filter-label">Compartir en</label>
          <div class="social-row">
            <button @click="shareTo('whatsapp')" class="social-btn social-wa" aria-label="Compartir en WhatsApp" title="WhatsApp">
            ...
            <button @click="shareTo('telegram')" class="social-btn social-tg" aria-label="Compartir en Telegram" title="Telegram">
            ...
            <button @click="shareTo('twitter')" class="social-btn social-x" aria-label="Compartir en X" title="X">
              <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
            </button>
            <button @click="shareTo('email')" class="social-btn social-em" aria-label="Compartir por email" title="Email">
    ...
    <div v-if="cloneModalVisible" class="modal-overlay" @click.self="closeCloneModal">
      <div class="glass-modal confirm-modal">
        <button class="modal-close" @click="closeCloneModal" aria-label="Cerrar">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M6 18 18 6M6 6l12 12"/></svg>
        </button>
        <h3 class="modal-title">Clonar lista</h3>
        <p class="modal-text">¿Crear una copia de &ldquo;<strong>{{ list.name }}</strong>&rdquo; en tu colección?</p>
        <div class="modal-actions">
          <button @click="closeCloneModal" class="btn-ghost">Cancelar</button>
          <button @click="confirmCloneList" class="btn-primary">Confirmar</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="renameListModalVisible" class="modal-overlay" @click.self="closeRenameModal">
      <div class="glass-modal rename-modal">
        <button class="modal-close" @click="closeRenameModal" aria-label="Cerrar">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M6 18 18 6M6 6l12 12"/></svg>
        </button>
        <h3 class="modal-title">Renombrar colección</h3>
        <div class="field-group">
          <label class="filter-label">Nuevo nombre</label>
          <input type="text" v-model="newListName" class="field-input" @keyup.enter="updateListName" autofocus placeholder="ej. Mejores Películas">
        </div>
        <div class="field-group">
          <label class="filter-label">Descripción (opcional)</label>
          <textarea v-model="newListDescription" class="field-textarea" placeholder="Añadí una descripción…"></textarea>
        </div>
        <div class="modal-actions">
          <button @click="closeRenameModal" class="btn-ghost">Cancelar</button>
          <button @click="updateListName" class="btn-primary" :disabled="!newListName || !newListName.trim()">Guardar</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
    ...
    computed: {
        ...
        sortOptions() {
           return [
             { value: 'latest-added', label: 'Recién agregados' },
             { value: 'earliest-added', label: 'Primeros agregados' },
             { value: 'newer-releases', label: 'Estrenos recientes' },
             { value: 'older-releases', label: 'Estrenos antiguos' },
             { value: 'imdb-high', label: 'Mejor calificados (IMDB/TMDB)' },
             { value: 'imdb-low', label: 'Peor calificados (IMDB/TMDB)' },
             { value: 'votes-high', label: 'Más votados' },
             { value: 'votes-low', label: 'Menos votados' },
             { value: 'shortest-first', label: 'Más cortos primero' },
             { value: 'longest-first', label: 'Más largos primero' }
           ];
        },
        yearRanges() {
        ...
        currentSortLabel() {
           ...
           return option ? option.label : 'Recién agregados';
        },
        activeFilterChips() {
           const chips = [];
           if (this.selectedGenre) chips.push({ label: this.selectedGenre, value: 'selectedGenre' });
           if (this.customYearStart) chips.push({ label: `Desde ${this.customYearStart}`, value: 'customYearStart' });
           if (this.customYearEnd) chips.push({ label: `Hasta ${this.customYearEnd}`, value: 'customYearEnd' });
           ...
           if (this.selectedUserRating) {
               const label = this.selectedUserRating === 'not-rated' ? 'Sin calificar' : `Mi calificación: ${this.selectedUserRating}`;
               chips.push({ label, value: 'selectedUserRating' });
           }

           if (this.orderMode !== 'latest-added') chips.push({ label: `Orden: ${this.currentSortLabel}`, value: 'orderMode' });
           return chips;
        },
    ...
    methods: {
        ...
        async handleCloneList() {
             ...
             if (!userEmail) {
                 if (typeof window !== 'undefined') {
                     ...
                     alert('Debés iniciar sesión para clonar listas.');
                 }
                 return;
             }
         ...
         async confirmCloneList() {
             ...
             try {
                ...
                if (response.ok) {
                    ...
                    alert(`No se pudo clonar la lista: ${errorMessage}`);
                }
             } catch (e) {
                 console.error(e);
                 alert('Ocurrió un error al clonar la lista.');
             } finally {
                 this.loading = false;
             }
        },
        ...
        async saveRatingAndReview() {
          if (this.selectedRating === 0) {
            alert('Seleccioná una calificación entre 1 y 10');
            return;
          }
          ...
          try {
            ...
            alert('No se pudo guardar la calificación. Probá de nuevo.');
          }
        },
