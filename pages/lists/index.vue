  ...
  <div class="lists-page">
    ...
    <main class="main-content">
      <header class="page-hero">
        ...
        <p class="page-subtitle">Organize your favorite movies and TV shows into custom collections.</p>
      </header>
      ...
      <div v-else-if="lists.length === 0" class="state-card">
        ...
        <p>Curate your own collections of movies and TV shows.</p>
      ...
      <section v-else class="list-panel">
        <div class="panel-toolbar">
          <div v-if="shouldShowFilter" class="seg-control" role="radiogroup" aria-label="Filter lists">
            ...
            <label for="filter-private">Private</label>
          ...
          <span v-else class="panel-count">{{ lists.length }} {{ lists.length === 1 ? 'list' : 'lists' }}</span>
        ...
        <div v-if="filteredLists.length === 0" class="state-card inset">
          <h3>No {{ filterMode }} lists found</h3>
          <p>Switch the filter to see the rest of your lists.</p>
        </div>
          ...
          <article
            ...
            <div class="card-cover">
              ...
              <span class="privacy-badge" :class="{ 'is-public': list.is_public }">
                ...
                <span class="privacy-label">{{ list.is_public ? 'Public' : 'Private' }}</span>
              </span>

              <div v-if="editingListId !== list.id" class="card-actions">
                <button type="button" @click.stop="startEdit(list)" class="icon-btn" aria-label="Edit list" title="Edit">
                ...
                <button type="button" @click.stop="deleteList(list)" class="icon-btn danger" aria-label="Delete list" title="Delete">
            ...
            <div class="card-body">
              <div v-if="editingListId === list.id" class="edit-form" @click.stop>
                <input v-model="editForm.name" class="field-input" placeholder="List name" @keyup.enter="saveEdit" @keyup.esc="cancelEdit" autoFocus />
                <textarea v-model="editForm.description" class="field-input field-textarea" placeholder="Description" rows="2"></textarea>
                ...
                <div class="edit-actions">
                  <button type="button" @click.stop="cancelEdit" class="btn-ghost">Cancel</button>
                  <button type="button" @click.stop="saveEdit" class="btn-primary" :disabled="!editForm.name || !editForm.name.trim()">Save</button>
                </div>
              </div>
              ...
              <template v-else>
                ...
                <span class="item-count">{{ list.item_count || 0 }} {{ list.item_count === 1 ? 'item' : 'items' }}</span>
              </template>
            </div>
          </article>
        </div>
      ...
      <transition name="slide-up">
        <div v-if="undoList" class="undo-banner" role="status">
          <span class="undo-text">List &ldquo;{{ undoList.name }}&rdquo; deleted</span>
          <button type="button" @click="undoDelete" class="undo-btn">Undo</button>
