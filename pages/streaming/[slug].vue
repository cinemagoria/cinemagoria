<template>
  <main class="main">
    ...
    <div class="sp-toolbar">
      ...
      <div class="sp-genres" role="group" aria-label="Filtrar por género">
        <button
          type="button"
          class="sp-genre"
          :class="{ 'is-on': selectedGenre === '' }"
          @click="pickGenre('', '')">Todos</button>
        <button
      ...
      <div v-if="hasActiveFilters" class="sp-active">
        ...
        <button type="button" class="sp-clear-all" @click="clearAllFilters">Limpiar todo</button>
      </div>
    </div>
    ...
    <section class="spacing">
      <div v-if="activeTab === 'movies'" class="content-section">
        ...
        <div v-else-if="!moviesLoading" class="sp-empty">
          ...
          <h3 v-else>Todavía no hay nada para mostrar</h3>
          <p v-if="hasActiveFilters">Basta con aflojar uno o dos filtros para que la lista vuelva a llenarse.</p>
          <p v-else>No hay películas encontradas para este proveedor de streaming.</p>
          <button v-if="hasActiveFilters" type="button" class="sp-btn sp-btn--ghost sp-empty__action" @click="clearAllFilters">Limpiar filtros</button>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'tvShows'" class="content-section">
        ...
        <div v-else-if="!tvShowsLoading" class="sp-empty">
          <img src="/ui/cinema-popcorn.svg" alt="" class="sp-empty__art">
          <h3 v-if="hasActiveFilters">No hay series que coincidan con tus filtros</h3>
          <h3 v-else>Todavía no hay nada para mostrar</h3>
          <p v-if="hasActiveFilters">Basta con aflojar uno o dos filtros para que la lista vuelva a llenarse.</p>
    ...
    <div v-if="filtersModalVisible" class="sp-modal-overlay" @click="closeFiltersModal">
      <div class="sp-modal" role="dialog" aria-modal="true" aria-labelledby="sp-modal-title" @click.stop>
        <button type="button" class="sp-modal__close" aria-label="Cerrar" @click="closeFiltersModal">
        ...
        <div class="sp-modal__head">
          <h3 id="sp-modal-title">Afinar resultados</h3>
          <p>Los géneros están a un toque en la barra. Estos ajustan la búsqueda todavía más.</p>
        </div>

        <div class="sp-modal__body">
          <div class="sp-field">
            <span class="sp-field__label">Años de estreno</span>
            <div class="sp-range">
              <input type="number" v-model.number="customYearStart" :min="1880" :max="currentYear" placeholder="Desde" aria-label="Año desde">
              <em>a</em>
              <input type="number" v-model.number="customYearEnd" :min="1880" :max="currentYear" placeholder="Hasta" aria-label="Año hasta">
            </div>
            <div class="sp-quick">
          ...
          <div class="sp-field">
            <span class="sp-field__label">Calificación</span>
            <div class="sp-range">
              <input type="number" v-model.number="minImdbRating" min="0" max="10" step="0.1" placeholder="Mín" aria-label="Calificación mínima">
              <em>a</em>
              <input type="number" v-model.number="maxImdbRating" min="0" max="10" step="0.1" placeholder="Máx" aria-label="Calificación máxima">
            </div>
          </div>

          <div class="sp-field">
            <span class="sp-field__label">Votos</span>
            <div class="sp-range">
              <input type="number" v-model.number="minImdbVotes" min="0" placeholder="Mín" aria-label="Votos mínimos">
              <em>a</em>
              <input type="number" v-model.number="maxImdbVotes" min="0" placeholder="Máx" aria-label="Votos máximos">
            </div>
          </div>

          <div class="sp-field">
            <span class="sp-field__label">Ordenar por</span>
            <div class="sp-sort">
              <button
        ...
        <div class="sp-modal__foot">
          <button type="button" class="sp-btn sp-btn--ghost" @click="clearAllFilters">Limpiar</button>
          <button type="button" class="sp-btn sp-btn--solid" @click="applyFilters">Ver resultados</button>
        </div>
      </div>
    </div>
...
const movieGenres = [
  ...
  { id: 36, name: "Historia" }, { id: 27, name: "Horror" }, { id: 10402, name: "Música" },
...
const tvGenres = [
  ...
  { id: 10763, name: "Noticias" }, { id: 10764, name: "Realidad" }, { id: 10765, name: "Ciencia ficción & Fantasía" },
  { id: 10766, name: "Soap" }, { id: 10767, name: "Talk" }, { id: 10768, name: "Guerra & Política" },
  { id: 37, name: "Western" }
];
...
const sortOptions = computed(() => {
    ...
    return [
        ...
        { value: `${dateField}.desc`, label: 'Nuevos lanzamientos' },
        { value: `${dateField}.asc`, label: 'Lanzamientos antiguos' },
        { value: 'vote_average.desc', label: 'Calificación más alta' },
        { value: 'vote_average.asc', label: 'Calificación más baja' },
        { value: 'vote_count.desc', label: 'Mayor Cantidad de Votos' },
        { value: 'vote_count.asc', label: 'Menor Cantidad de Votos' }
    ];
});
...
const resultCount = computed(() => {
    ...
    if (!total) return '';
    return `${total.toLocaleString('es-AR')} ${activeTab.value === 'movies' ? 'películas' : 'series'}`;
});
