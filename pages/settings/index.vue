      ...
      <div class="settings-container">
        ...
        <div class="settings-content">
          <div class="avatar-section">
            <div class="avatar-wrapper" @click="openAvatarModal">
              ...
              <div class="avatar-overlay">
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M12 5v14m-7-7h14"/></svg>
              </div>
            </div>
            <button @click="openAvatarModal" class="change-avatar-button">
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M12 20h9M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4z"/></svg>
          ...
          <div class="user-details-card">
            ...
            <div class="detail-item">
              <div class="detail-label">
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><path d="m22 6-10 7L2 6"/></svg>
                <span>Correo</span>
              </div>
          ...
          <div class="settings-card">
            <div class="settings-card-header">
              ...
              <p>Configurá tu alias, bio y preferencias de privacidad.</p>
            </div>
            <div class="settings-card-body">
              ...
              <div class="toggle-row">
                <div>
                  <span class="toggle-label">Mostrar mis reseñas públicamente</span>
                  <span class="toggle-desc">Otros pueden ver tus reseñas calificadas en tu perfil.</span>
                </div>
              ...
              <div class="toggle-row">
                <div>
                  <span class="toggle-label">Mostrar mis listas públicamente</span>
                  <span class="toggle-desc">Otros pueden ver tus listas públicas en tu perfil.</span>
                </div>
                <label class="toggle-switch">
          ...
          <div class="actions-container">
            <div class="danger-zone">
              ...
              <button @click="deleteAccount" class="action-button danger">
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M3 6h18m-2 0v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
        ...
        <div class="avatar-grid">
          <div 
            ...
            <div class="avatar-check" v-if="avatar === userAvatar">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24"><path d="M20 6 9 17l-5-5"/></svg>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="delete-modal">
        ...
        <div class="delete-modal-content">
          <div class="exclamation-svg">
            <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="none" stroke="#ff6b6b" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path d="M12 8v4m0 4h.01"/></svg>
          </div>
          <p class="delete-message">¿CONFIRMA LA ELIMINACIÓN PERMANENTE DE SU CUENTA?</p>
          <p class="delete-warning">Su cuenta entrará en proceso de eliminación. Tenga en cuenta que el cierre de la cuenta asociada a <strong>{{ userEmail }}</strong> será irreversible.</p>
          <p class="delete-info">Tiene 72 horas para revertir la acción comunicándose a <strong>hello@entercinema.com</strong>. Si no hay solicitud de restablecimiento en ese plazo, la cuenta se eliminará definitivamente.</p>
          
          <div class="delete-actions">
            <button @click="closeDeleteModal" class="cancel-btn" type="button">
              <span class="label-button-modal">Cancelar</span>
            </button>
            <button @click="confirmDelete" class="confirm-delete-btn" type="button">
              <span class="label-button-modal">Confirmar</span>
            </button>
          </div>
        </div>
      </div>
  ...
  methods: {
    ...
    async saveProfile() {
      ...
      if (aliasVal && !/^[a-zA-Z0-9_-]{2,30}$/.test(aliasVal)) {
        this.aliasError = 'El alias debe tener 2-30 caracteres: letras, números, guion bajo o guión.';
        return;
      }
      ...
      try {
        ...
        if (resp.ok) {
          this.aliasSuccess = '¡Perfil guardado!';
          ...
          this.aliasError = err.error || 'No se pudo guardar el alias. Puede que ya esté tomado.';
        }
      } catch(e) {
        this.aliasError = 'Error de red.';
      } finally {
...
.label-button-modal { margin: 0 auto; }

.settings-card {
  background: rgba(0,0,0,0.25); border-radius: 12px;
  border: 1px solid rgba(139,233,253,0.12); margin-bottom: 2rem; overflow: hidden;
}
.settings-card-header { padding: 1.5rem 2rem 1rem; border-bottom: 1px solid rgba(139,233,253,0.08); }
.settings-card-header h3 { color: #8BE9FD; font-size: 1.6rem; font-weight: 600; margin: 0 0 0.4rem 0; }
.settings-card-header p { color: rgba(255,255,255,0.55); font-size: 1.25rem; margin: 0; }
.settings-card-body { padding: 1.5rem 2rem; }
.field-row { margin-bottom: 1.4rem; }
.field-row label { display: block; font-size: 1.3rem; color: rgba(255,255,255,0.65); margin-bottom: 0.5rem; }
...
.input-with-prefix {
  ...
  background: rgba(0,0,0,0.35); border: 1px solid rgba(139,233,253,0.2);
  border-radius: 6px; overflow: hidden;
}
.prefix { padding: 0.8rem 1rem; color: #8BE9FD; font-size: 1.4rem; background: rgba(139,233,253,0.07); border-right: 1px solid rgba(139,233,253,0.15); }
.settings-input { flex: 1; background: transparent; border: 1px solid rgba(139,233,253,0.2); border-radius: 6px; color: #fff; font-size: 1.35rem; padding: 0.8rem 1rem; width: 100%; }
.input-with-prefix .settings-input { border: none; border-radius: 0; }
textarea.settings-input { resize: vertical; min-height: 70px; }
.field-error { color: #FF6B6B; font-size: 1.2rem; margin: 0.4rem 0 0; }
.field-success { color: #8BE9FD; font-size: 1.2rem; margin: 0.4rem 0 0; }
.settings-divider {
  border: none;
...
.toggle-row { display: flex; justify-content: space-between; align-items: center; padding: 1rem 0; border-bottom: 1px solid rgba(255,255,255,0.06); gap: 1.5rem; }
.toggle-row:last-of-type { border-bottom: none; }
.toggle-label { display: block; font-size: 1.35rem; color: rgba(255,255,255,0.85); margin-bottom: 0.2rem; }
.toggle-desc { display: block; font-size: 1.15rem; color: rgba(255,255,255,0.45); }
.toggle-switch { position: relative; display: inline-block; width: 44px; height: 24px; flex-shrink: 0; }
.toggle-switch input { opacity: 0; width: 0; height: 0; position: absolute; }
.toggle-slider { position: absolute; cursor: pointer; top: 0; left: 0; right: 0; bottom: 0; background: rgba(255,255,255,0.15); border-radius: 24px; transition: background 0.2s; }
...
.toggle-switch input:checked + .toggle-slider { background: #8BE9FD; }
