<template>
  <main class="page">
    ...
    <header class="hero">
      <div class="heroInner">
        <span class="eyebrow">Ayuda &amp; Consultas</span>
        <h1 class="heroTitle">Soporte y Contacto</h1>
        <p class="heroLead">
          Consultas sobre la cuenta, una watchlist, un título faltante o cualquier otra inquietud relacionada con la plataforma.
        </p>
      </div>
    </header>
        ...
        <form @submit.prevent="submitForm" class="contactForm" novalidate>
          <transition name="state-fade" mode="out-in">
            <div v-if="state === 'success'" key="success" class="successState">
              ...
              <h2 class="successTitle">Mensaje recibido</h2>
              <p class="successText">El mensaje fue entregado. La respuesta llegará pronto al correo proporcionado.</p>
              <button type="button" class="ghostBtn" @click="resetForm">Enviar otro</button>
            </div>

            <div v-else key="form" class="formBlock">
              <div class="fieldRow">
                <div class="fieldGroup">
                  <label for="name" class="fieldLabel">Nombre</label>
                  <input
                    id="name"
                    v-model="form.name"
                    type="text"
              ...
              <div class="fieldGroup">
                <label for="subject" class="fieldLabel">Asunto</label>
                <input
                  id="subject"
              ...
              <div class="fieldGroup">
                <label for="message" class="fieldLabel">Mensaje</label>
                <textarea
                  id="message"
                  v-model="form.message"
                  class="fieldTextarea"
                  placeholder="Describí la consulta &mdash; cuanto más específica, más rápido se puede atender."
                  rows="6"
                  maxlength="5000"
                  required
                ></textarea>
...
useHead({
  ...
  meta: [
    { name: 'description', content: 'Comunicate con el equipo de la plataforma por consultas sobre la cuenta, listas, títulos faltantes, datos de festivales o cobertura editorial.' },
    { property: 'og:title', content: 'Soporte y Contacto' },
  ],
});
...
async function submitForm() {
  ...
  try {
    ...
    errorMessage.value = 'No se pudo enviar el mensaje. Probá de nuevo en un momento.';
  } finally {
    loading.value = false;
  }
}
