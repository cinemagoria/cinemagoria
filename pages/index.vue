...
const { data: pageData, error: pageError } = useAsyncData('homepage', async () => {
  try {
    ...
    const FESTIVAL_SLUGS = ['sundance','berlinale','rotterdam','slamdance','sxsw','romford','bifff','bafici','cannes','tribeca','cuff','kviff','fantasia','frightfest','venice','tiff'];
    ...
    const allFestivalFilms = [...sundanceList, ...berlinaleList, ...rotterdamList, ...slamdanceList, ...sxswList, ...romfordList, ...bifffList, ...baficiList, ...cannesList, ...tribecaList, ...cuffList, ...kviffList, ...fantasiaList, ...frightfestList, ...veniceList, ...tiffList];
