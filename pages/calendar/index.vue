  ...
  <div class="calendar-page">
    ...
    <main class="cal-main">
      <header class="page-header cal-header">
        ...
        <p class="page-subtitle">Theatrical, streaming and festival dates, day by day.</p>
        ...
        <button class="cal-scope-btn" @click="scopeOpen = true">
          ...
          Which titles appear here
        </button>
      </header>

      <section class="cal-panel cal-nav-panel">
        <div class="cal-nav">
          <button class="cal-step" :aria-label="`Previous ${view}`" @click="step(-1)">
          ...
          <div class="cal-period">
            ...
            <span class="cal-period-sub">{{ periodEntries.length }} releases</span>
          </div>

          <button class="cal-step" :aria-label="`Next ${view}`" @click="step(1)">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" aria-hidden="true"><path d="M9 5l7 7-7 7" /></svg>
          </button>

          <div class="cal-modes" role="tablist" aria-label="Calendar view">
            <button
              v-for="mode in MODES"
              :key="mode.id"
              role="tab"
          ...
          <button v-if="!isCurrentPeriod" class="cal-today" @click="goToday">Today</button>
        </div>

        <div v-if="view === 'week'" class="cal-strip" role="tablist" aria-label="Day">
          <button
          ...
          >
            <span class="cal-strip-name">All</span>
      ...
      <section class="cal-panel cal-filters" aria-label="Filters">
        <div class="cal-lenses" role="group" aria-label="Release type">
          <button
        ...
        <div class="cal-controls">
          <div class="cal-search">
            ...
            <input v-model="query" type="search" placeholder="Search this period" aria-label="Search titles" />
            <button v-if="query" class="cal-clear" aria-label="Clear search" @click="query = ''">
          ...
          <div ref="territoryBox" class="control-group control-group--territory">
            <span class="control-label">Territory</span>
            <button
          ...
          <button v-if="hasActiveFilters" class="cal-reset" @click="resetFilters">Reset</button>
        </div>
      </section>

      <div v-if="pending" class="cal-state">
        <div class="cal-spinner" aria-hidden="true"></div>
        <p>Loading {{ periodLabel }}</p>
      </div>

      <div v-else-if="loadError" class="cal-state error">
        <p>{{ loadError }}</p>
        <button class="cal-retry" @click="loadPeriod(true)">Try again</button>
      </div>
      ...
      <template v-else>
        <section v-for="group in shownGroups" :key="group.date" class="cal-group">
          <header class="cal-group-head">
            ...
            <span v-if="group.date === todayIso" class="cal-group-today">Today</span>
        ...
        <p v-if="!shownGroups.length" class="cal-empty">
          {{ hasActiveFilters ? 'Nothing matches these filters.' : 'No releases recorded for this period.' }}
        </p>
      </template>
    </main>
...
const MODES = [{ id: 'month', label: 'Month' }, { id: 'week', label: 'Week' }]
const WEEKDAYS = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']
const LENSES = [
  { id: 'all', label: 'All', types: null },
  { id: 'theatrical', label: 'Theaters', types: [2, 3] },
  { id: 'streaming', label: 'Streaming', types: [4] },
  { id: 'festival', label: 'Festivals', types: [1] },
...
const periodLabel = computed(() => {
  ...
  const a = start.toLocaleDateString('en-US', sameYear
    ? { month: 'long', day: 'numeric', timeZone: 'UTC' }
    : { month: 'long', day: 'numeric', year: 'numeric', timeZone: 'UTC' })
  const b = end.toLocaleDateString('en-US', sameYear
    ? (sameMonth ? { day: 'numeric', timeZone: 'UTC' } : { month: 'long', day: 'numeric', timeZone: 'UTC' })
    : { month: 'long', day: 'numeric', year: 'numeric', timeZone: 'UTC' })
  return sameYear ? `${a} – ${b}, ${end.getUTCFullYear()}` : `${a} – ${b}`
})
...
async function loadPeriod(force = false) {
  ...
  try {
    ...
    loadError.value = 'Could not load this period.'
  } finally {
    pending.value = false
  }
}
...
const territoryLabel = computed(() => {
  if (!territory.value) return 'Worldwide'
