<template>
  <main class="main">
    <UserNav />
    <TopNav title="Histórico de N.O.I.R" />
    ...
    <section class="noir-archive">
      ...
      <transition name="fade">
        <div v-if="cloneSuccess" class="undoBarContainer">
          <div class="undoBar">
            <span>&iexcl;Colecci&oacute;n creada!</span>
            <nuxt-link v-if="newlyCreatedListSlug" :to="`/lists/${newlyCreatedListSlug}`" class="viewLinkButton">Ver</nuxt-link>
      ...
      <transition name="fade">
        <div v-if="showReplaceConfirm" class="undoBarContainer">
          <div class="undoBar undoBar--confirm">
            <span>Ya clonaste N.O.I.R hoy. &iquest;Reemplazar?</span>
            <div class="confirm-actions">
              <button @click="confirmReplace" class="confirmButton confirmButton--yes">Reemplazar</button>
              <button @click="cancelReplace" class="confirmButton confirmButton--no">Cancelar</button>
            </div>
          </div>
        </div>
      </transition>
      ...
      <div v-if="!loading && hasResults" class="toolbar">
        ...
        <button @click="cloneToList" class="clone-button" :disabled="cloning">
          ...
          {{ cloning ? 'Clonando...' : 'Clonar a Colecci\u00F3n' }}
        </button>
      </div>
      ...
      <div v-else-if="hasResults">
        ...
        <div v-if="sortedMovies.length" class="noir-category">
          <div class="section-header" @click="collapsed.movies = !collapsed.movies">
            <h2 class="section-title">Pel&iacute;culas <span class="section-count">({{ sortedMovies.length }})</span></h2>
            <button class="expand-btn" :aria-label="collapsed.movies ? 'Expandir' : 'Contraer'">
          ...
          <div v-show="!collapsed.movies">
            ...
            <div v-if="moviesWithoutDate.length" class="no-date-separator">
              <span class="no-date-label">Fecha por confirmar</span>
            </div>
        ...
        <div v-if="sortedTv.length" class="noir-category">
          <div class="section-header" @click="collapsed.tv = !collapsed.tv">
            <h2 class="section-title">Series de TV <span class="section-count">({{ sortedTv.length }})</span></h2>
            <button class="expand-btn" :aria-label="collapsed.tv ? 'Expandir' : 'Contraer'">
      ...
      <div v-else class="empty-state">
        <p>A&uacute;n no hay t&iacute;tulos archivados.</p>
      </div>
    </section>
  ...
  methods: {
      ...
      try {
        ...
        if (listsRes.ok) {
          ...
          const existingNoir = (userLists.lists || userLists || []).find(l => {
            if (l.name !== 'Hist\u00F3rico de N.O.I.R') return false;
    ...
    async confirmReplace() {
      ...
      try {
        ...
        this.cloneError = 'Error al reemplazar la colecci\u00F3n existente.';
        setTimeout(() => { this.cloneError = null; }, 4000);
        this.cloning = false;
      }
    },
    ...
    async _createNoirList(userEmail) {
      ...
      try {
        ...
        const createRes = await fetch(`${this.tursoBackendUrl}/lists`, {
          ...
          body: JSON.stringify({
            userEmail,
            name: 'Hist\u00F3rico de N.O.I.R',
            description: `T\u00EDtulos Hist\u00F3ricos Seleccionados Para N.O.I.R hasta ${today}`,
            isPublic: false,
            ownerName,
          }),
        });
