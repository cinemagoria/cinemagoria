        ...
        <div class="news-main">
          <div class="news-toolbar" :class="{ 'search-active': isSearchActive }">
            <div class="toolbar-left">
              ...
              <NuxtLink v-if="userEmail" :to="{ path: '/news', query: { view: 'saved' } }" class="saved-articles-link" :class="{ 'active': isSavedView }" aria-label="Artículos Guardados">
          ...
          <div class="header-status">
            ...
            <h2 class="status-title" v-else-if="selectedSource">
              ...
              >
                {{ selectedSource }}
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" class="external-link-icon" viewBox="0 0 24 24"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6m4-3h6v6m-11 5L21 3"/></svg>
              </a>
            </h2>
            ...
            <ClientOnly>
              <span class="count-badge" v-if="isSavedView">
                {{ localSavedArticlesList.length }} {{ localSavedArticlesList.length === 1 ? 'artículo' : 'artículos' }}
              </span>
              <span class="count-badge" v-else-if="!pending">
                {{ filteredItems.length }} {{ filteredItems.length === 1 ? 'artículo' : 'artículos' }}
              </span>
            </ClientOnly>
          </div>

          <!-- Filtro por categoría editorial — debajo del header status para
               anclar primero el conteo del feed, ofrecer el filtro después.
               Chips solo de texto, dentro del panel glassmorphism. -->
          <div v-if="showCategoryChips && !isSavedView" class="category-panel" role="tablist" aria-label="Filtrar por categoría editorial">
            <button
              type="button"
              class="category-chip"
                  ...
                  <div v-else class="no-results">
                      <div style="text-align: center;">
                        <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#8BE9FD" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" style="opacity: 0.5; margin-bottom: 20px;"><path d="m19 21-7-4-7 4V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v16z"/><line x1="12" x2="12" y1="7" y2="13"/><line x1="15" x2="9" y1="10" y2="10"/></svg>
                         <h3 style="color: #8BE9FD; font-size: 16px; margin-bottom: 10px;">Aún no hay artículos salvados.</h3>
                        <p style="font-size: 14px; color: #aaa;">Los artículos que guardes aparecerán aquí.</p>
                      </div>
                  </div>
               </div>
...
function pickCategory(cat) {
  categoryFilter.value = cat; // null = "Todas"
...
const groupedSavedArticles = computed(() => {
  ...
  localSavedArticlesList.value.forEach(article => {
    const source = article.source || 'Fuente Desconocida';
    if (!groups[source]) {
      groups[source] = [];
    }
