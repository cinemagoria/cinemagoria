<template>
  ...
  <main class="main">
            ...
            <div class="article-card">
              <header class="article-header">
                ...
                <div v-if="article.image_url" class="mobile-hero">
                  <img :src="article.image_url" :alt="article.title_en" loading="eager" />
                </div>
                  ...
                  <div class="related-entities-list">
                    ...
                    >
                      <img
                        ...
                        loading="lazy"
                      />
                      <img v-else src="/placeholders/image_not_found_yet.webp" :alt="entity.name" class="related-entity-img" loading="lazy" />
              ...
              <div v-if="article.carousel_assets?.length && !article.trailer_youtube_id && !isGated" class="article-carousel">
                <div class="carousel-viewport">
                  ...
                  <template v-if="article.carousel_assets.length > 1">
                    <button class="carousel-btn carousel-btn-prev" :disabled="carouselIndex === 0" @click="carouselIndex--" aria-label="Previous image">
                    ...
                    <button class="carousel-btn carousel-btn-next" :disabled="carouselIndex === article.carousel_assets.length - 1" @click="carouselIndex++" aria-label="Next image">
    ...
    <!-- Share modal (rendered once; teleports to body) -->
    <ArticleShareModal
      v-if="article"
...
async function fetchRelatedEntities() {
  ...
  // title_overrides — same hierarchy as the rest of the app (Details.js mixin):
...
// Unlisted/private Vimeo videos additionally require the privacy hash
// (trailer_vimeo_hash) as an `h` param — without it the player refuses to load.
...
// so the carousel can be inserted in the middle when both trailer and
// carousel exist. Falls back to a single part (carousel at the end) when
// there are no <h2> headings or when the condition isn't met.
...
useHead(() => {
  ...
  const jsonLd = {
    ...
    '@type': 'NewsArticle',
    headline: article.value.title_en,
    description: article.value.description_en,
    image: article.value.image_url ? [article.value.image_url] : ['https://cinemagoria.com/og-image.jpg'],
    datePublished: article.value.published_at,
    dateModified: article.value.published_at,
    author: [{ '@type': 'Organization', name: 'Cinemagoria', url: 'https://cinemagoria.com' }],
    publisher: {
      '@type': 'Organization',
      name: 'Cinemagoria',
      url: 'https://cinemagoria.com',
      logo: { '@type': 'ImageObject', url: 'https://cinemagoria.com/icons/favicon-192.png' }
    },
    mainEntityOfPage: { '@type': 'WebPage', '@id': `https://cinemagoria.com/news/${article.value.slug}` },
...
.not-found-title {
  ...
  letter-spacing: -0.3px;
  margin-bottom: 8px;
  text-shadow: 0 0 18px rgba(139, 233, 253, 0.25);
}
...
@media (max-width: 900px) {
  ...
  /* Show inline hero inside article card */
  .mobile-hero {
    display: block;
    margin: 16px 0;
    border-radius: 10px;
