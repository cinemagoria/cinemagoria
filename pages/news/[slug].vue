          ...
          <div class="article-main">
            <div class="article-card">
              ...
              <!-- Body — completo si es público o si el lector está logueado.
                   Reemplazado por una tarjeta inline cuando requires_auth = 1
                   y el lector es anónimo. La tarjeta corta el body entero
                   (sin teaser, sin fade) para que el gate sea visualmente
                   inmediato y no dependa del tiempo de apertura del modal. -->
              <template v-if="!isGated">
                <div class="article-body" v-html="bodyParts.before"></div>
              </template>
              <div v-else class="gate-card" role="region" aria-label="Artículo exclusivo para miembros">
              ...
              <!-- Carrusel de artículos relacionados (se arma a partir del footer markdown que el CMS inyecta en body_es) -->
              ...
              <div v-if="parsedSources.length" class="mobile-sources">
                <h3 class="mobile-sources-title">Fuentes</h3>
...
const relatedSlugs = computed(() => {
  return extractRelatedSlugs(article.value?.body_es || '')
})

const renderedBody = computed(() => {
  const raw = article.value?.body_es || ''
  if (!raw) return ''
...
.sidebar-meta-id { font-size: 12px; color: #5c6770; font-variant-numeric: tabular-nums; letter-spacing: 0.3px; }
...
.not-found-title { color: #fff; font-size: 22px; font-weight: 800; letter-spacing: -0.3px; margin-bottom: 8px; text-shadow: 0 0 18px rgba(139, 233, 253, 0.25); }
.not-found-text { font-size: 14px; color: #a0aab2; font-weight: 300; line-height: 1.55; margin-bottom: 28px; }

.back-btn {
  display: inline-flex; align-items: center; gap: 6px; padding: 11px 28px;
  background: linear-gradient(135deg, #1F5467, #8BE9FD); border: 1px solid rgba(139,233,253,0.5); border-radius: 10px;
  color: #03242C; font-size: 14px; font-weight: 600; text-decoration: none; transition: all 0.2s ease;
  box-shadow: 0 4px 14px rgba(139,233,253,0.18);
}

.back-btn:hover { transform: translateY(-1px); box-shadow: 0 6px 20px rgba(139,233,253,0.28); }
