      ...
      <div
        ...
        <div v-if="searchable" class="fs-menu__search">
          ...
          <input
            ...
            v-model="term"
            type="text"
            :placeholder="`Filtrar ${(label || 'opciones').toLowerCase()}`"
            :aria-label="`Filtrar ${(label || 'opciones').toLowerCase()}`"
        ...
        <div ref="list" class="fs-menu__options">
          ...
          <p v-if="searchable && !visibleOptions.length" class="fs-menu__empty">Sin coincidencias.</p>
        </div>
      </div>
    </Teleport>
  </div>
...
const props = defineProps({
  ...
  fallbackLabel: { type: String, default: 'Seleccionar' },
