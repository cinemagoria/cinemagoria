  ...
  <div>
    <div class="spacing" :class="$style.info">
      ...
      <div :class="$style.right">
        ...
        <div v-if="isLoggedIn" :class="$style.followSection">
            <h4 class="minor-title" style="text-transform: uppercase;">Notificaciones</h4>
            <button 
    ...
    <div v-if="ratingModalVisible" class="movie-info-modal-overlay" @click.self="closeRatingModal">
      <div class="movie-info-rating-modal">
        <div class="movie-info-modal-header">
          <h3>Valorar '{{ item.name || item.title }}'</h3>
...
export default {
  ...
  computed: {
    ...
    isReleasedOrCloseToRelease() {
      if (!this.item.first_air_date) return true;
      const releaseDate = new Date(this.item.first_air_date);
      const now = new Date();
  ...
  methods: {
    ...
    fullDate(date) {
      ...
      return new Date(y, m - 1, d).toLocaleDateString(process.env.API_COUNTRY === 'BR' ? 'pt-BR' : 'es-ES', {
        year: 'numeric',
        month: 'long',
        day: 'numeric'
      });
