<template>
  <div>
    <div class="spacing" :class="$style.info">
      ...
      <div :class="$style.right">
        ...
        <div v-if="isLoggedIn" :class="$style.followSection">
            <h4 class="minor-title" style="text-transform: uppercase;">Notifications</h4>
            <button 
    ...
    <div v-if="hasAnyRecommendations" class="recommendations-wrapper">
      <h2 :class="$style.title">Recommendations</h2>
...
export default {
  ...
  computed: {
    ...
    availableTabs() {
      ...
      if (this.creatorItems?.results?.length) tabs.push({ key: 'creator', label: 'From the Same Creator' });
      return tabs;
    },
    currentTabLabel() {
