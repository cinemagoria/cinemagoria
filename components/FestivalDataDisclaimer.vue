<template>
  <!-- Trigger de info minimalista, teleported al hero del festival (top-right),
       espejando el botón .back-link que va arriba a la izquierda del backdrop. -->
  <Teleport v-if="heroEl" :to="heroEl">
    <button
      class="hero-info-trigger"
      @click="openCoverage"
      aria-label="¿Por qué faltan algunas películas?"
      title="¿Por qué faltan algunas películas?"
    >
  ...
  <!-- Modal de cobertura del catálogo -->
  <Teleport to="body">
    <transition name="modal-fade">
      <div v-if="open" class="cov-overlay" @click.self="closeCoverage">
        <div class="cov-modal" role="dialog" aria-modal="true" aria-labelledby="cov-title">
          <button class="cov-close" @click="closeCoverage" aria-label="Cerrar">
          ...
          <h3 id="cov-title" class="cov-title">Cobertura del Catálogo</h3>

          <p class="cov-text">La cantidad de largometrajes y cortometrajes mostrados aquí puede no coincidir con la grilla oficial del festival. Nuestro catálogo se construye a partir de metadatos públicos y fuentes de terceros, que pueden no cubrir todos los títulos, en particular cortometrajes, obras experimentales o producciones regionales.</p>

          <p class="cov-text">Esto se debe a una limitación técnica, no a una decisión editorial. No censuramos ni omitimos intencionalmente ninguna película. <nuxt-link to="/usage-policies" target="_blank" class="accent-link">Leer políticas de uso completas</nuxt-link>.</p>
    ...
    <transition name="modal-fade">
      <div v-if="reportOpen" class="report-overlay" @click.self="closeReport">
        <div class="report-card" role="dialog" aria-modal="true" aria-labelledby="report-title">
          <button type="button" class="cov-close" @click="closeReport" aria-label="Cerrar">
            <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
          </button>

          <transition name="state-fade" mode="out-in">
            <div v-if="reportState === 'form'" key="form" class="state-block">
              <h2 id="report-title" class="report-title">Reportar un error</h2>
              <p class="report-subtitle">
                Ayudanos a corregir esta selección. Elegí qué está mal y contanos un poco más.
              </p>

              <form @submit.prevent="submitReport" class="report-form">
                <fieldset class="issue-type-group">
                  <legend class="field-label">Tipo de error</legend>
                ...
                <div class="field-group">
                  ...
                  <textarea
                    ...
                    placeholder="Describí qué falta o qué está mal — cuanto más específico, más rápido lo corregimos."
                    rows="4"
                    maxlength="2000"
                    required
                  ></textarea>
                ...
                <div class="field-row">
                  <div class="field-group">
                    <label for="festival-report-name" class="field-label">
                      Nombre <span class="field-optional">(opcional)</span>
                    </label>
                    <input
            ...
            <div v-else-if="reportState === 'success'" key="success" class="state-block success-block">
              ...
              <h2 class="success-title">Reporte recibido</h2>
              <p class="success-text">
                El reporte fue enviado. Será revisado y, si corresponde, la selección será corregida.
              </p>
            </div>
          </transition>
        </div>
...
const issueOptions = [
  { value: 'missing', label: 'Título faltante' },
  { value: 'incorrect', label: 'Título incorrecto' },
  { value: 'section_date', label: 'Sección o fecha' },
  { value: 'other', label: 'Otro' }
];

const canSubmit = computed(() => form.description.trim().length >= 5);

// El disclaimer solo se monta en páginas /festival/<slug>, así que la
// identidad del festival se lee directo de la ruta — sin pasar props por página.
...
async function submitReport() {
  ...
  try {
    ...
    if (res?.success) {
      ...
      errorMessage.value = res?.message || 'Algo salió mal. Probá de nuevo.';
    }
  } catch (err) {
    errorMessage.value = err?.data?.statusMessage || err?.message || 'No pudimos enviar el reporte. Intentá nuevamente.';
  } finally {
    submitting.value = false;
  }
}
...
onMounted(async () => {
  await nextTick();
  // Anclamos el trigger al hero del festival; si una página no tuviera hero,
  // el trigger simplemente no se renderiza (graceful — todas lo tienen).
...
<style scoped>
/* ── Trigger de info en el hero (espejo de .back-link, top-right) ── */
.hero-info-trigger {
  position: absolute;
  top: 30px;
  right: 30px;
...
/* ── Modal de cobertura (glassmorphism) ──────────────────────────── */
.cov-modal {
  position: relative;
...
/* ── Botón de cerrar — X roja, bien visible sobre la card oscura ── */
.cov-close {
  position: absolute;
  top: 14px;
  right: 14px;
...
<!--
  Override de layout no-scoped, conservado del componente original. El ancestro
  `.switcher-container { top: 3.5rem }` es un offset visual (position:relative) que
  no empuja a sus hermanos, así que el switcher + WinnersCarousel debajo se solapan.
  Matamos ese offset y lo reemplazamos por margin de flujo para un ritmo limpio.
  Se carga en cada página de festival porque este componente se monta en todas.
-->
<style>
