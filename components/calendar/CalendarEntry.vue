...
const kindLabel = computed(() => KIND_LABEL[props.entry.type] || 'Release')
...
const flags = computed(() => {
  const out = []
  if (props.entry.rerelease) out.push('re-release')
  if (props.entry.short) out.push('short')
  if (props.entry.precision === 'year') out.push('day unconfirmed')
  if (props.entry.confidence === 'speculative') out.push('unannounced')
  else if (props.entry.confidence === 'estimated') out.push('estimated')
  return out
})
</script>
