...
export default {
  ...
  data() {
    return {
      ...
      sortOptions: [
        { value: 'newest-releases', label: 'Newest Releases' }, 
        { value: 'oldest-releases', label: 'Oldest Releases' }, 
        { value: 'imdb-high', label: 'IMDb Rating: High to Low' },
        { value: 'imdb-low', label: 'IMDb Rating: Low to High' },
        { value: 'imdb-votes-high', label: 'IMDb Votes: High to Low' },
        { value: 'imdb-votes-low', label: 'IMDb Votes: Low to High' },
      ],
  ...
  methods: {
    ...
    async fetchFollowedContent() {
      ...
      try {
        ...
        if (followedItems && followedItems.length > 0) {
          ...
          this.companySections = resolvedSections.filter(s => s);
        }
      } catch (error) {
    ...
    async loadMore(companyId) {
        ...
        try {
            ...
            const apiFunc = this.getApiFunction();
            const response = await apiFunc(companyId, nextPage);
    ...
    getFilteredItems(items) {
      ...
      let result = items.filter(item => {
        ...
        if (typeof rating === 'string') rating = rating.replace(',', '.');
      ...
      result.sort((a, b) => {
         const dateField = this.type === 'movie' ? 'release_date' : 'first_air_date';
