<template>
  <nuxt-link
    to="/festival/cannes-2026"
    class="cannes-banner"
    role="banner"
    aria-label="Ganadores de Cannes 2026 — explorar el palmarès"
  >
    <!-- Background: cinemagoria-dark with subtle radial glows -->
    <svg
      class="cannes-banner__bg"
      viewBox="0 0 1200 80"
      preserveAspectRatio="xMidYMid slice"
      aria-hidden="true"
    >
      <defs>
        <linearGradient id="cannes-win-bg-grad" x1="0" y1="0" x2="1" y2="0">
          <stop offset="0%"   stop-color="#000000" />
          <stop offset="50%"  stop-color="#060c0f" />
          <stop offset="100%" stop-color="#000000" />
        </linearGradient>
      </defs>
      <rect width="1200" height="80" fill="url(#cannes-win-bg-grad)" />
    </svg>
    ...
    <div class="cannes-banner__inner">
      <!-- Left: brand (logo + pill badge) -->
      <div class="cannes-banner__brand">
        <!-- Official Cannes logo (white, transparent bg) -->
        <svg
          class="cannes-banner__logo"
          viewBox="0 0 234.078 117.007"
        ...
        <!-- Pill -->
        <div class="cannes-banner__pill">
          <svg class="cannes-banner__pill-star" width="9" height="9" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 2l2.5 7.5h7.5l-6 4.5 2.5 7.5-6.5-4.8-6.5 4.8 2.5-7.5-6-4.5h7.5z"/></svg>
          PALMARÈS · 2026
        </div>
      </div>

      <!-- Center: animated marquee ticker OR tagline fallback -->
      <div class="cannes-banner__ticker-wrap" aria-hidden="true">
        <div v-if="marqueeItems.length" class="cannes-banner__ticker">
          <span
            v-for="(item, i) in marqueeItems"
            :key="'win-' + i"
            class="cannes-banner__win"
            :class="{ 'is-grand': item.grand }"
          >
            <svg v-if="item.grand" class="cannes-banner__win-star" width="9" height="9" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 2l2.5 7.5h7.5l-6 4.5 2.5 7.5-6.5-4.8-6.5 4.8 2.5-7.5-6-4.5h7.5z"/></svg>
            <span class="cannes-banner__win-award">{{ item.label }}</span>
            <span class="cannes-banner__win-title">{{ item.title }}</span>
            <span class="cannes-banner__win-sep" aria-hidden="true">•</span>
          </span>
        ...
        <div v-else class="cannes-banner__tagline">
          Ganadores del 79.º Festival de Cannes —
          <span class="cannes-banner__tagline-accent">explorá el palmarès completo</span>
        </div>
      </div>

      <!-- Right: CTA -->
      <span class="cannes-banner__cta" aria-label="Ver ganadores de Cannes">
        Explorar
      </span>
    </div>
  </nuxt-link>
...
const { data: awardsData } = await useFetch('/api/festival/cannes/awards', {
  default: () => ({ results: [] }),
});

const awards = computed(() => awardsData.value?.results ?? []);

const marqueeItems = computed(() =>
  awards.value.map((a) => ({
    label: a.award_name,
    title: a.title,
    grand: !!a.is_grand_prize,
  }))
);
</script>
...
.cannes-banner__shimmer {
  ...
  animation: cannes-shimmer-sweep 5.5s ease-in-out infinite;
  pointer-events: none;
}

@keyframes cannes-shimmer-sweep {
  0%   { background-position: -120% 0; }
  100% { background-position: 220% 0; }
}

/* ─── Inner Layout ─────────────────────────────────────────────────── */
.cannes-banner__inner {
  position: relative;
  z-index: 3;
  display: flex;
  align-items: center;
...
/* ─── Brand ────────────────────────────────────────────────────────── */
.cannes-banner__brand {
  display: flex;
  align-items: center;
  gap: 16px;
  flex-shrink: 0;
  padding-right: 18px;
  border-right: 1px solid rgba(139, 233, 253, 0.18);
}
...
.cannes-banner__logo {
  ...
  animation: cannes-logo-glow 4.5s ease-in-out infinite;
}

@keyframes cannes-logo-glow {
  0%, 100% {
...
.cannes-banner__pill {
  ...
  align-items: center;
  gap: 6px;
  background: rgba(240, 196, 45, 0.08);
  border: 1px solid rgba(240, 196, 45, 0.4);
  color: #f0c42d;
  font-size: 9.5px;
  font-weight: 700;
...
.cannes-banner__pill-star {
  flex-shrink: 0;
  filter: drop-shadow(0 0 4px rgba(240, 196, 45, 0.5));
}

/* ─── Ticker Marquee ───────────────────────────────────────────────── */
.cannes-banner__ticker-wrap {
  flex: 1;
  overflow: hidden;
  height: 100%;
  display: flex;
...
.cannes-banner__ticker {
  display: flex;
  align-items: center;
  white-space: nowrap;
  animation: cannes-ticker-scroll 42s linear infinite;
  will-change: transform;
}

@keyframes cannes-ticker-scroll {
...
.cannes-banner__win-star {
  color: #f0c42d;
  flex-shrink: 0;
  filter: drop-shadow(0 0 4px rgba(240, 196, 45, 0.5));
}

.cannes-banner__win-award {
  color: #8BE9FD;
  font-weight: 600;
}

.cannes-banner__win.is-grand .cannes-banner__win-award {
  color: #f0c42d;
}

.cannes-banner__win-title {
  color: rgba(255, 255, 255, 0.92);
  font-weight: 400;
}

.cannes-banner__win-sep {
  color: rgba(139, 233, 253, 0.4);
  margin-left: 14px;
}

.cannes-banner__tagline {
  color: rgba(255, 255, 255, 0.82);
  font-size: 12.5px;
  font-weight: 400;
