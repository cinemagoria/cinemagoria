<template>
  <div v-if="visible" :class="$style.overlay" @click.self="close">
    <div :class="$style.modal" role="dialog" aria-modal="true" aria-labelledby="create-list-title">
      ...
      <h2 id="create-list-title" :class="$style.title">Crear nueva colección</h2>
      <p :class="$style.lead">Agrupa películas y series en una colección propia.</p>
      ...
      <form @submit.prevent="createList" :class="$style.form">
        <div :class="$style.field">
          ...
          <input id="listName" type="text" v-model="form.name" :class="$style.input" required maxlength="90" autocomplete="off" placeholder="ej. Mejor Ciencia Ficción 2024" />
        </div>

        <div :class="$style.field">
          <label for="listDesc" :class="$style.label">
            <span>Descripción <em :class="$style.optional">opcional</em></span>
            <span :class="[$style.count, { [$style.countLow]: 180 - form.description.length < 10 }]">{{ 180 - form.description.length }}</span>
          </label>
          <textarea id="listDesc" v-model="form.description" rows="3" maxlength="180" :class="[$style.input, $style.textarea]" placeholder="¿De qué trata esta colección?"></textarea>
        </div>

        <label :class="$style.switchRow">
          <span :class="$style.switchText">
            <span :class="$style.switchTitle">Colección pública</span>
            <span :class="$style.switchHint">Cualquiera con el enlace puede verla.</span>
          </span>
