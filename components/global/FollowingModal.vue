      ...
      <div :class="$style.modalContent">
        ...
        <div :class="$style.tabs">
          <button 
            ...
            <span :class="$style.tabLabel">People ({{ peopleCount }})</span>
          </button>
          <button 
            @click="activeTab = 'tv'" 
            :class="[{ [$style.active]: activeTab === 'tv' }]">
            <span :class="$style.tabLabel">TV Series ({{ tvCount }})</span>
          </button>
          <button 
...
export default {
  ...
  methods: {
    ...
    getUndoText() {
      ...
      if (this.undoItem.type === 'streaming') return `${this.undoItem.provider_name} unfollowed`;
