<template>
  <div class="discover-search">
    <p v-if="!selectedGenre" class="default-note">Select a type and genre to discover</p>
...
function applyClientSort(items, sortValue) {
  ...
  const getVotes = item => {
    if (item.imdb_votes) return typeof item.imdb_votes === 'string' ? parseInt(item.imdb_votes.replace(/,/g, ''), 10) : Number(item.imdb_votes) || 0;
...
function getMinVotesLabel(val) { return minVoteOptions.find(o => o.value === val)?.label || `${val}+ Votes`; }
function getSortLabel(value) { return sortOptions.find(o => o.value === value)?.label || 'Highest Rated'; }
...
function selectSort(value) {
  ...
  if (isClientSort(value) && results.value.length > 0) {
...
async function fetchResults(page = 1, reset = false) {
  ...
  try {
    ...
    if (clientSort) {
      ...
      totalPages.value = allData[0].total_pages || 1;
      ...
      for (let i = 0; i < allResults.length; i += CHUNK_SIZE) {
        ...
        enriched.push(...await Promise.all(chunk.map(item => enrichItem(item, type))));
      }
      ...
      if (ratingMin.value || ratingMax.value) {
        ...
        finalResults = enriched.filter(item => { const r = item.imdb_rating ? parseFloat(item.imdb_rating) : parseFloat(item.vote_average); return r >= min && r <= max; });
      }
    ...
    console.error('DiscoverSearch fetch error:', err);
  } finally {
    loading.value = false;
  }
}
...
.filter-cell {
  ...
  &--clear { justify-content: flex-end; }
  &--disabled { opacity: 0.4; pointer-events: none; }
}
...
.select-display {
  ...
  &:hover { border-color: rgba(127, 219, 241, 0.5); color: #fff; }
  &--active { border-color: #7FDBF1; color: #7FDBF1; }
}
...
.dropdown-option {
  ...
  &--selected { color: #7FDBF1; background: rgba(127, 219, 241, 0.08); }
}
...
.input-wrap {
  ...
  &:focus-within { border-color: #7FDBF1; box-shadow: 0 0 0 2px rgba(127, 219, 241, 0.15); color: #fff; }
  &:hover { border-color: rgba(127, 219, 241, 0.5); }
...
.clear-btn {
  ...
  &:hover { background: rgba(255, 60, 60, 0.25); border-color: rgba(255, 60, 60, 0.5); }
}
...
.load-more-btn {
  ...
  &:hover { background: rgba(127, 219, 241, 0.1); border-color: #7FDBF1; }
}
