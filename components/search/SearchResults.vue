  ...
  <div class="listing">
    ...
    <div v-if="localNews && localNews.length > 0" class="news-section">
      <div class="section-header" @click="toggleSection('news')">
        ...
        <button class="expand-btn" :aria-label="collapsedSections.news ? 'Expandir' : 'Contraer'">
    ...
    <CategoryCarousel title="Películas" :items="movies" key-prefix="movie" :has-more="hasMore" :loading="loading" @loadMore="$emit('loadMore')" />
    <CategoryCarousel title="Series de TV" :items="tvShows" key-prefix="tv" :has-more="hasMore" :loading="loading" @loadMore="$emit('loadMore')" />
    <CategoryCarousel title="Personas" :items="people" key-prefix="person" :has-more="hasMore" :loading="loading" @loadMore="$emit('loadMore')" />
    <CategoryCarousel title="Servicios de Streaming" :items="streamingServices" key-prefix="streaming" :has-more="hasMore" :loading="loading" @loadMore="$emit('loadMore')" />
    <CategoryCarousel title="Productoras" :items="productionCompanies" key-prefix="company" :has-more="hasMore" :loading="loading" @loadMore="$emit('loadMore')" />
  ...
  methods: {
    ...
    async checkForTypos() {
      ...
      try {
        ...
        const response = await axios.post('https://cinemagoria-assistant-746175915741.us-east1.run.app/api/typo', {
    ...
    async fetchUserResults(q) {
      ...
      try {
        ...
        const resp = await fetch(`https://cinemagoria-follows-746175915741.us-east1.run.app/user-search?q=${encodeURIComponent(q.trim())}&limit=6`);
        if (resp.ok) {
