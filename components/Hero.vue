  ...
  <div
    ...
    <div
      ...
      <button
        ...
        :aria-label="autoAdvancePaused ? 'Reanudar auto-avance' : 'Pausar auto-avance'"
        :title="autoAdvancePaused ? 'Reanudar' : 'Pausar'"
          ...
          <div>
            ...
            <div :class="$style.meta">
              <div
                ...
                <div v-if="heroItem.rating_source === 'imdb' && heroItem.imdb_rating" :class="$style.ratingText">
                  ...
                  <span :class="$style.ratingVotes">{{ (heroItem.imdb_votes || 0).toLocaleString('es-ES') }} votos</span>
                ...
                <div v-else-if="heroItem.vote_average" :class="$style.ratingText">
                  ...
                  <span :class="$style.ratingVotes">{{ (heroItem.vote_count || 0).toLocaleString('es-ES') }} reseñas</span>
            ...
            <div :class="[$style.buttonContainer, { 'no-transition': isHomepage && !isHomepageContentReady }]">
              ...
              <button
                ...
                <span class="txt">{{ hasUserRating ? userRatingForDb : 'Valorar' }}</span>
              </button>
              ...
              <button
                ...
                <span class="icon">
                  <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" fill="none" viewBox="0 0 24 24"><path fill="currentColor" d="M18 16.08c-.76 0-1.44.3-1.96.77l-7.12-4.21c.05-.25.08-.51.08-.78s-.03-.53-.08-.78l7.12-4.21c.53.48 1.22.77 1.96.77 1.66 0 3-1.34 3-3s-1.34-3-3-3-3 1.34-3 3c0 .27.03.53.08.78L7.96 9.63c-.53-.48-1.22-.77-1.96-.77-1.66 0-3 1.34-3 3s1.34 3 3 3c.74 0 1.43-.29 1.96-.77l7.12 4.21c-.05.25-.08.51-.08.78 0 1.66 1.34 3 3 3s3-1.34 3-3-1.34-3-3-3"/></svg>
                </span>
              </button>
            </div>
          </div>
    ...
    <div v-if="ratingModalVisible" :class="$style.rateOverlay" @click.self="closeRatingModal">
      <div :class="$style.rateModal" role="dialog" aria-modal="true" aria-labelledby="hero-rate-title">
        <button type="button" :class="$style.rateClose" @click="closeRatingModal" aria-label="Cerrar">
        ...
        <h3 :class="$style.rateTitle" id="hero-rate-title">Valorar &lsquo;{{ nameForDb }}&rsquo;</h3>

        <div :class="$style.rateScore">
          <span :class="$style.rateScoreValue">{{ hoverRating || selectedRating || '–' }}</span>
          <span :class="$style.rateScoreLabel">{{ (hoverRating || selectedRating) ? 'sobre 10' : 'Elige una nota' }}</span>
        </div>
        ...
        <div v-if="type === 'movie'" :class="$style.pgSection">
          ...
          <div :class="$style.pgRow">
            ...
            <div :class="$style.pgControls">
              ...
              <input v-else type="range" :class="$style.pgSlider" min="0" max="100" step="1" v-model.number="progressPercentage" aria-label="Porcentaje visto" />

              <div v-if="heroItem.runtime" :class="$style.pgTimes">
                <div :class="$style.pgTime">
                  <span :class="$style.pgTimeLabel">Visto</span>
                ...
                <div :class="[$style.pgTime, $style.pgTimeRight]">
                  <span :class="$style.pgTimeLabel">Restante</span>
                  <span :class="$style.pgTimeValue">{{ progressRemaining }}</span>
                </div>
              </div>
              <p v-else :class="$style.pgNoDuration">Duración no disponible</p>
            </div>
          </div>
        </div>
        ...
        <div :class="$style.rateReview">
          <textarea
            v-model="userReview"
            :placeholder="selectedRating > 0 ? 'Añade una reseña opcional…' : 'Selecciona una valoración primero'"
        ...
        <div :class="$style.rateActions">
          <button v-if="hasUserRating" type="button" @click="removeRating" :class="[$style.rateGhost, $style.rateDanger]">Remover</button>
          <button type="button" @click="saveRatingAndReview" :class="$style.ratePrimary">Guardar</button>
        </div>
      </div>
    </div>
  ...
  computed: {
    ...
    activeFestivals() {
      const festivalConfig = [
        ...
        // Badge Selección de la Crítica tiene precedencia sobre badge regular Cannes
    ...
    trackingInfoText() {
      if (this.type === 'movie') {
        return `Progreso: ${this.progressPercentage}%`;
      }
      const seasons = this.trackedSeasonData;
      if (!seasons || seasons.length === 0) {
        return `${this.trackedEpisodesCount} ${this.trackedEpisodesCount !== 1 ? 'episodios' : 'episodio'} registrados`;
      }
      if (seasons.length === 1) {
        const s = seasons[0];
  ...
  methods: {
    ...
    async saveRatingAndReview() {
      try {
        ...
        if (this.type === 'movie') await this.saveProgress();
